...
class Utils(object):
    ...
    def __init__(self):
        ...
        self.pageNumS = "index_page01"  # 总页数的div名字
        self.pageTableName = "index_articl_list"   # 每条数据的div名字
        self.compilePageTable = '<li.*?><a.*?href="(.*?)".*?title="(.*?)".*?>.*?</a> <span>(.*?)</span></li>' # 每条数据的匹配规则：标题、时间
        self.ZCJDSavePath = "F:\知识产权局相关附件\政策解读\%s"
        self.ZSCQGZSavePath = "F:\知识产权局相关附件\知识产权工作\%s"
    ...
    def compareAndDown(self,ZCJDurl,ZCJDbaseUrl,ZCJDSavePath,ZSCQGZurl,ZSCQGZbaseUrl,ZSCQGZSavePath,DownBaseUrl):
        print("注意这里是比较下载的时候，因为会读取政策解读和知识产权工作全部网页的src耗时比较久，请耐心等.........")
            ...
            if ZCJDsrcList[0] ==  ZCJD上一次抓取的第一条数据url:
                ...
                for ZCJDids, ZCJDsrc1 in enumerate(ZCJDsrcList):
                    ...
                    if ZCJDsrc1 != ZCJD上一次抓取的第一条数据url:
                        ...
                        ZCJD来源处唯一标志_url, ZCJD来源处完整的url, ZCJDContent = self.parePage(ZCJDsrc1, ZCJDbaseUrl, ZCJDtitle,DownBaseUrl, ZCJDSavePath)
                        print(ZCJDsrc1)
                        ZCJDdate = "发布时间："+str(ZCJDtime)
                        ZCJDtypeName, ZCJDtype = "政策解读", 2
                        上一次整个抓取后的最大唯一标识 += 1
                        sql = "insert into [cnlaw2.0].[dbo].[知识产权-科新局](title,date,content,typeName,type,packid,插入时间,来源处的唯一标志_url,来源处完整的url) values('%s','%s','%s','%s','%s','%s','%s','%s','%s')"%(ZCJDtitle,ZCJDdate,ZCJDContent,ZCJDtypeName,int(ZCJDtype),int(packid1),本次更新时间,ZCJD来源处唯一标志_url,ZCJD来源处完整的url)
                        print(sql)
                ...
                政策解读本次抓取的第一条的标题的url = self.parePage(ZCJDsrcList[0], ZCJDbaseUrl, ZCJDtitle, DownBaseUrl,ZCJDSavePath)[0]
                政策解读本次抓取的第一条的标题的完整的url = self.parePage(ZCJDsrcList[0], ZCJDbaseUrl, ZCJDtitle, DownBaseUrl,ZCJDSavePath)[1]
                政策解读本次抓取的最后一条的标题的url =  self.parePage(ZCJDsrcList[ZCJDids-1], ZCJDbaseUrl, ZCJDtitle, DownBaseUrl,ZCJDSavePath)[0]
                政策解读本次抓取的最后一条的标题的完整的url = self.parePage(ZCJDsrcList[ZCJDids-1], ZCJDbaseUrl, ZCJDtitle, DownBaseUrl,ZCJDSavePath)[1]
            ...
            if ZSCQGZsrcList[0] == ZSCQGZ上一次抓取的第一条数据url:
                ...
                for ZSCQGZids, ZSCQGZsrc1 in enumerate(ZSCQGZsrcList):
                    ...
                    if ZSCQGZsrc1 != ZSCQGZ上一次抓取的第一条数据url:
                        ...
                        ZSCQGZsrc1, ZSCQGZtitle, ZSCQGZtime = ZSCQGZsrc1, ZSCQGZtitleList[ZSCQGZids], ZSCQGZtimeList[ZSCQGZids]
                        print("2  :")
                        print(str(ZSCQGZsrc1))
                        print(str(ZSCQGZtitle))
                        ZSCQGZ来源处唯一标志_url, ZSCQGZ来源处完整的url, ZSCQGZContent = self.parePage(ZSCQGZsrc1, ZSCQGZbaseUrl,ZSCQGZtitle,DownBaseUrl, ZSCQGZSavePath)
                        ...
                        sql = "insert into [cnlaw2.0].[dbo].[知识产权-科新局](title,date,content,typeName,type,packid,插入时间,来源处的唯一标志_url,来源处完整的url) values('%s','%s','%s','%s','%s','%s','%s','%s','%s')" % (
                            ZSCQGZtitle, ZSCQGZdate, ZSCQGZContent, ZSCQGZtypeName, int(ZSCQGZtype), int(packid1), 本次更新时间, ZSCQGZ来源处唯一标志_url, ZSCQGZ来源处完整的url)
                        print(sql)
                        self.writeTableRecod(sql)
                ...
                知识产权工作本次抓取的第一条的标题的url = self.parePage(ZSCQGZsrcList[0], ZSCQGZbaseUrl,ZSCQGZtitle,DownBaseUrl, ZSCQGZSavePath)[0]
                知识产权工作本次抓取的第一条的标题的完整的url = self.parePage(ZSCQGZsrcList[0], ZSCQGZbaseUrl, ZSCQGZtitle, DownBaseUrl, ZSCQGZSavePath)[1]
                知识产权工作本次抓取的最后一条的标题的url = self.parePage(ZSCQGZsrcList[ZSCQGZids - 1], ZSCQGZbaseUrl,ZSCQGZtitle,DownBaseUrl, ZSCQGZSavePath)[0]
                知识产权工作本次抓取的最后一条的标题的完整的url = self.parePage(ZSCQGZsrcList[ZSCQGZids - 1], ZSCQGZbaseUrl,ZSCQGZtitle,DownBaseUrl, ZSCQGZSavePath)[1]
        ...
        sql = "insert into [cnlaw2.0].[dbo].[知识产权局更新记录表](政策解读上一次抓取的第一条的标题,政策解读上一次抓取的第一条标题的发布时间,政策解读上一次抓取的第一条的标题的url,政策解读上一次抓取的第一条的标题的完整的url,政策解读上一次抓取的第一条的标题的唯一标志ID,知识产权工作上一次抓取的第一条的标题,知识产权工作上一次抓取的第一条的标题的发布时间,知识产权工作上一次抓取的第一条的标题的url,知识产权工作上一次抓取的第一条的标题的完整的url,知识产权工作上一次抓取的第一条的标题的唯一标志ID,packid,上一次整个抓取后的最大的唯一标志ID,政策解读本次是否有更新,知识产权工作本次是否有更新,本次更新时间) values('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')"%( 政策解读本次抓取的第一条的标题,政策解读本次抓取的第一条的标题的发布时间 ,政策解读本次抓取的第一条的标题的url ,政策解读本次抓取的第一条的标题的完整的url ,int(政策解读本次抓取的第一条的标题的唯一标志ID ),知识产权工作本次抓取的第一条的标题 , 知识产权工作本次抓取的第一条的标题的发布时间 ,知识产权工作本次抓取的第一条的标题的url, 知识产权工作本次抓取的第一条的标题的完整的url ,int(知识产权工作本次抓取的第一条的标题的唯一标志ID) , int(packid+1) ,int(本次整个抓取后的最大唯一标志),政策解读本次是否有更新 ,知识产权工作本次是否有更新 , 本次更新时间)
        print(sql)
        self.writeTableRecod(sql)
		 
    # 每次在更新的时候去查询数据表（知识产权局表）中的上一次结束后政策解读和知识产权工作的结束id和最后一条数据的标题、时间、url、完整的url、以及上一次结束时的最大ID
    def readTableRecord(self):
        # 查询政策解读上一次抓取的第一条数据的 标题 、发布时间、 唯一标识、src 、完整的url
        sql1 = """
               select title,date,唯一标志,来源处的唯一标志_url,来源处完整的url from [cnlaw2.0].[dbo].[知识产权-科新局]   where  唯一标志=(select min(唯一标志)
           from  [cnlaw2.0].[dbo].[知识产权-科新局] where date = (select max([date])FROM [cnlaw2.0].[dbo].[知识产权-科新局]  where typeName='政策解读'))
               """

        # 查询知识产权工作上一次抓取的第一条数据的 标题 、发布时间、 唯一标识、src 、完整的url
        sql2 = """
        select title,date,唯一标志,来源处的唯一标志_url,来源处完整的url from [cnlaw2.0].[dbo].[知识产权-科新局]   where  唯一标志=(select min(唯一标志)
    from  [cnlaw2.0].[dbo].[知识产权-科新局] where date = (select max([date])FROM [cnlaw2.0].[dbo].[知识产权-科新局]  where typeName='科信'))
        """

        # 查询上一次抓取后的最大唯一标识，插入数据的时候不需要插入，因为它是自增的，用于备份表记录
        sql3 = """
            select max(唯一标志) from [cnlaw2.0].[dbo].[知识产权-科新局]
        """
        sql4 = """
            select max(packid) from [cnlaw2.0].[dbo].[知识产权-科新局]
        """
        ...
        for i in sql:
            ...
            rs.append(row)
        print(rs)
        ZCJD上一次抓取的第一条标题 = rs[0][0].replace("\r", '').replace(" ", '').replace("\n",'')
        ZCJD上一次抓取的第一条发布时间 = rs[0][1].replace("\r", '').replace(" ", '').replace("\n",'')
        ZCJD上一次抓取的第一条唯一标识 = rs[0][2]
        ZCJD上一次抓取的第一条url = rs[0][3].replace("\r", '').replace(" ", '').replace("\n",'')
        ZCJD上一次抓取的第一条完整的url = rs[0][4].replace("\r", '').replace(" ", '').replace("\n",'')
        ZSCQGZ上一次抓取的第一条标题 = rs[1][0].replace("\r", '').replace(" ", '').replace("\n",'')
        ZSCQGZ上一次抓取的第一条发布时间 = rs[1][1].replace("\r", '').replace(" ", '').replace("\n",'')
        ZSCQGZ上一次抓取的第一条唯一标识 = rs[1][2]
        ZSCQGZ上一次抓取的第一条url = rs[1][3].replace("\r", '').replace(" ", '').replace("\n",'')
        ZSCQGZ上一次抓取的第一条完整的url = rs[1][4].replace("\r", '').replace(" ", '').replace("\n",'')
        上一次抓取后的最大唯一标识 = rs[2][0]
        上一次抓取后的packid = rs[3][0]
        链接数据库.breakConnect(conn)
        return ZCJD上一次抓取的第一条标题, ZCJD上一次抓取的第一条发布时间, ZCJD上一次抓取的第一条唯一标识, ZCJD上一次抓取的第一条url, ZCJD上一次抓取的第一条完整的url, ZSCQGZ上一次抓取的第一条标题, ZSCQGZ上一次抓取的第一条发布时间, ZSCQGZ上一次抓取的第一条唯一标识, ZSCQGZ上一次抓取的第一条url, ZSCQGZ上一次抓取的第一条完整的url, 上一次抓取后的最大唯一标识, 上一次抓取后的packid
...
if __name__ =="__main__":
    # 数据库:
    # 账号：192.168.31.124\SQLEXPRESS  密码： 123qwe!@#    数据库：cnlaw2.0 表：知识产权-科新局
    # 在该表中使用typeName字段来区分：知识产权工作-----科信，知识产权政策解读----政策解读
    # 在该表中使用type字段:type=1就是科信，type=2就是政策解读
    # 知识产权工作 http://www.sipo.gov.cn/zscqgz/index.htm  相关附件保存位置：F:\知识产权局相关附件\知识产权工作
    # 政策解读 http://www.sipo.gov.cn/zcfg/zcjd/index.htm   相关附件保存位置：F:\知识产权局相关附件\政策解读
    AdminiStrative =Utils()
    # 政策解读相关参数
    ZCJDurl = "http://www.sipo.gov.cn/zcfg/zcjd/index.htm"
    ZCJDbaseUrl = "http://www.sipo.gov.cn/zcjd/"
    ZCJDSavePath = "F:\知识产权局相关附件\政策解读\%s"
    # 知识产权工作相关参数
    ZSCQGZurl = "http://www.sipo.gov.cn/zscqgz/index.htm"
    ZSCQGZbaseUrl ="http://www.sipo.gov.cn/zscqgz/"
    ZSCQGZSavePath = "F:\知识产权局相关附件\知识产权工作\%s"
    # 文章中图片下载的基础地址
    DownBaseUrl = "http://www.sipo.gov.cn/"
    AdminiStrative.compareAndDown(ZCJDurl,ZCJDbaseUrl,ZCJDSavePath,ZSCQGZurl,ZSCQGZbaseUrl,ZSCQGZSavePath,DownBaseUrl)
    time.sleep(2)
