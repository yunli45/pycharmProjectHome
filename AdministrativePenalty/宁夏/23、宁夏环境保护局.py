...
from 宁夏.工具包 import 链接数据库, 替换标签, 附件下载程序, 只有行政处罚信息公开表,判断全文的类型返回表中需要的字段,表格加全文的形式8行19格10行23格,_23用全文没有表格
...
class Utils(object):
    ...
    def parsePage(self, url=None, pageNo=None, baseUrl=None, SavePath=None,RSaddress=None,RSarea=None,RSFalseHeadlines=None):
        ...
        responseList = responseList.find_all('div', attrs={'class': 'news-items-r-list'})
        print(responseList)
        RSlist = re.findall(re.compile(r'<li id=".*?">.*?<a.*?href="(.*?)".*?><span></span>(.*?)</a><em>(.*?)</em>', re.S | re.M),str(response))
        print(RSlist)
        print(len(RSlist))
        SrcList = []
        TitleList = []
        ...
        for src in SrcList:
            ...
            ConetentResponseSoupOld = ConetentResponseSoup.find('div', attrs={'id': 'vsb_newscontent'})
            ...
            fields = _23用全文没有表格.getNeed(ConetentResponseSoupOld, ConetentResponse, ContentPNum, ContentPNum1, ContentTrNum, ContentTdNum,
            ...
            # 假如存在数据，则保存数据道数据库
            if ConetentResponse:
                sql1 = " INSERT INTO  AdministrativePunNingXia.dbo.crawlData18(dataId,title,documentNum,bePunished,principal,lawEnforcement,punishedDate,content,uniqueSign,address,area,agency,grade,showId,FalseHeadlines,Page,sourceUrl) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s') " % (
...
if __name__ =="__main__":
    # 共计1页数据
    url ="http://www.nxep.gov.cn/zwgk2/jczf/xzcf.htm"
    baseUrl="http://www.nxep.gov.cn"
    SavePath = "F:\行政处罚数据\宁夏\宁夏工商局\%s"
    RSaddress ="宁夏回族自治区"
    RSarea =" " # 在全文的地址栏去匹配
    RSFalseHeadlines ="宁夏回族自治区的环境保护厅数据"
