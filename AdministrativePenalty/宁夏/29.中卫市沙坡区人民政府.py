...
from 宁夏.工具包 import 链接数据库, 替换标签, 附件下载程序, _29全是图片
...
class Utils(object):
    ...
    def parsePage(self, url=None, pageNo=None, baseUrl=None, SavePath=None,RSaddress=None,RSarea=None,RSFalseHeadlines=None):
        ...
        responseList = responseList.find_all('div',attrs={'class':'xxgkml-item'})
        # print(responseList)
        RSlist = re.findall (re.compile(r'<li><a.*?href="(.*?)".*?>(.*?)</a><span>(.*?)</span></li>', re.S | re.M),str(response))
        print(RSlist)
        print(len(RSlist))
        SrcList = []
        TitleList = []
        ...
        for src in SrcList:
            ...
            OnlyID = self.OnlyID
            header = self.header
            fields =_29全是图片.getNeed(ConetentResponseSoupOld, ConetentResponse, ContentPNum, ContentPNum1, ContentTrNum, ContentTdNum,
            ...
            # 假如存在数据，则保存数据道数据库
            if ConetentResponse:
                sql1 = " INSERT INTO  AdministrativePunNingXia.dbo.crawlData24(dataId,title,documentNum,bePunished,principal,lawEnforcement,punishedDate,content,uniqueSign,address,area,agency,grade,showId,FalseHeadlines,Page,sourceUrl) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s') " % (
...
if __name__ =="__main__":
    # 共计1页数据
    url ="http://www.spt.gov.cn/xxgk/xzcfjggk/index"
    baseUrl="http://www.spt.gov.cn/xxgk/xzcfjggk"
    SavePath = "F:\行政处罚数据\宁夏\中卫市沙坡区人民政府\%s"
    RSaddress ="宁夏回族自治区"
    RSarea ="中卫市沙坡区" # 在全文的地址栏去匹配
    RSFalseHeadlines ="宁夏回族自治区中卫市沙坡区人民政府的数据"
