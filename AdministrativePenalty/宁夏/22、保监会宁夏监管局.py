...
from 宁夏.工具包 import 链接数据库, 替换标签, 附件下载程序,_22行政处罚信息公示表,_22替换标签
...
class Utils(object):
    ...
    def __init__(self):
        ...
        self.OnlyID = 1
        self.showId = 12374809
        # 运行到第4页的时候卡住了 12374437  8 :12374777
        # 程序卡住的原因： 这一页有多条数据（比如第四页第9条16个等了好久，第15条有29个）
    ...
    def parsePage(self, url=None, pageNo=None, baseUrl=None, SavePath=None,RSaddress=None,RSarea=None,RSFalseHeadlines=None):
        ...
        responseList = responseList.find_all('div', attrs={'id': 'div111'})
        print(responseList)
        RSlist = re.findall(re.compile(r'<span>(.*?)</span> <a href="(.*?)".*?>(.*?)</a>', re.S | re.M),str(responseList))
        RSlist = RSlist[:20]
        print(RSlist)
        print(len(RSlist))
        SrcList = []
        ...
        for src in SrcList:
            ...
            ConetentResponseSoupOld = ConetentResponseSoup.find('div', attrs={'id': 'cont'})
            # 返回去除掉部分标签的全文、返回去掉标签前原来的tr 和td的个数 、返回去掉表格前后p标签的个数
            returnData = _22替换标签.replaceLable(ConetentResponseSoupOld)
            ...
            ConetentResponse = _22替换标签.replaceLableNow(ConetentResponseSoupOld)

            OnlyID = self.OnlyID
            header= self.header

            fields = _22行政处罚信息公示表.publicTable(ConetentResponseSoupOld, ConetentResponse, ContentPNum, ContentPNum1, ContentTrNum, ContentTdNum,
            ...
            RSlawEnforcementList = fields[3]  # 处罚机构
            RSpunishedDateList = fields[4]  # 处罚时间
            RScontentList =fields[5]
            RSagencyList = fields[6]  # 处罚机构
            RSTitleList = fields[7]
            print(RSdocumentNumList)
            for i in RSdocumentNumList:
                RSdataId = RSdataId
                RSTitle = re.sub("<font.*?>",'',RSTitle).replace("</font>",'')
                RSTitle = RSTitleList[RSdocumentNumList.index(i)]
                RSdocumentNum =  i # 书文号
                RSbePunished =RSbePunishedList[RSdocumentNumList.index(i)]   # 被处罚人或者单位
                RSprincipal = RSprincipalList[RSdocumentNumList.index(i)] # 法人
                RSlawEnforcement =  RSlawEnforcementList[RSdocumentNumList.index(i)] # 处罚机构
                RSpunishedDate = RSpunishedDateList[RSdocumentNumList.index(i)]   # 处罚时间
                RScontent = RScontentList[RSdocumentNumList.index(i)] # 全文
                RsuniqueSign = ContentSrc
                RSagency =RSagencyList[RSdocumentNumList.index(i)]   # 处罚机构
                RSaddress = RSaddress
                RSarea = RSarea
            ...
            # 假如存在数据，则保存数据道数据库
                if ConetentResponse:
                    sql1 = " INSERT INTO  AdministrativePunNingXia.dbo.crawlData17(dataId,title,documentNum,bePunished,principal,lawEnforcement,punishedDate,content,uniqueSign,address,area,agency,grade,showId,FalseHeadlines,Page,sourceUrl) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s') " % (
...
if __name__ =="__main__":
    # 共计353页
    url ="http://www.nxfda.gov.cn/html/ajgg/index"
    baseUrl="http://www.nxfda.gov.cn"
    SavePath = "F:\行政处罚数据\宁夏\宁夏食品监管局\%s"
    RSaddress ="宁夏回族自治区"
    RSarea =" "
    RSFalseHeadlines ="宁夏回族自治区的食品监管局数据"
