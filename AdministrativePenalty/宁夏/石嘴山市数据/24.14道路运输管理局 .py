...
class Utils(object):
    ...
    def __init__(self):
        ...
        self.OnlyID = 1
        self.showId =12377605
        # self.showId =12375913  跑到53页27条，这一页剩下的数据全部没有数据直接从54开始跑
    ...
    def parsePage(self, url=None,params=None, pageNo=None, baseUrl=None, SavePath=None,RSaddress=None,RSarea=None,RSFalseHeadlines=None):
        ...
        for src in SrcList:
            ...
            # 假如存在数据，则保存数据道数据库
            if ConetentResponse:
                sql1 = " INSERT INTO  AdministrativePunNingXia.dbo.crawlData19(dataId,title,documentNum,bePunished,principal,lawEnforcement,punishedDate,content,uniqueSign,address,area,agency,grade,showId,FalseHeadlines,Page,sourceUrl) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s') " % (
...
if __name__ =="__main__":
    ...
    SavePath = "F:\行政处罚数据\宁夏\石嘴山市\%s"
    RSaddress ="宁夏回族自治区"
    RSarea ="石嘴山市" # 在全文的地址栏去匹配
    RSFalseHeadlines ="宁夏回族自治区石嘴山市道路运输管理局的数据"
