...
from 宁夏.工具包 import 链接数据库, 替换标签, 附件下载程序, 只有行政处罚信息公开表,判断全文的类型返回表中需要的字段
...
class Utils(object):
    ...
    def __init__(self):
        ...
        self.OnlyID = 1
        # self.showId = 12370200
        self.showId = 12370220
    ...
    def parsePage(self, url=None, pageNo=None, baseUrl=None, SavePath=None,RSaddress=None,RSarea=None,RSFalseHeadlines=None):
        if pageNo == "1":
            response = self.getPage("http://www.ngsh.gov.cn/zfxxgs/zzqgsj/List_2.html")
        else:
            response = self.getPage("http://www.ngsh.gov.cn/zfxxgs/zzqgsj/List_1.html")
        ...
        for src in SrcList:
            ...
            # 假如存在数据，则保存数据道数据库
            if ConetentResponse:
                sql1 = " INSERT INTO  crawlData2(dataId,title,documentNum,bePunished,principal,lawEnforcement,punishedDate,content,uniqueSign,address,area,agency,grade,showId,FalseHeadlines,Page,sourceUrl) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s') " % (
...
if __name__ =="__main__":
    ...
    RSaddress ="宁夏回族自治区"
    RSarea =""
    RSFalseHeadlines ="宁夏回族自治区的工商局数据"
