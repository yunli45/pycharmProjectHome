...
class Utils(object):
    ...
    def parsePage(self, url=None, pageNo=None, baseUrl=None, SavePath=None,RSaddress=None,RSarea=None,RSFalseHeadlines=None):
        ...
        for src in SrcList:
            ...
            # 假如存在数据，则保存数据道数据库
            if ConetentResponse:
                sql1 = " INSERT INTO  AdministrativePunNingXia.dbo.crawlData7(dataId,title,documentNum,bePunished,principal,lawEnforcement,punishedDate,content,uniqueSign,address,area,agency,grade,showId,FalseHeadlines,Page,sourceUrl) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s') " % (
...
if __name__ =="__main__":
    # 共计353页
    url ="http://www.ngsh.gov.cn/zfxxgs/xq2fj/List_"
    baseUrl="http://www.ngsh.gov.cn"
    SavePath = "F:\行政处罚数据\宁夏\宁夏工商局\%s"
    RSaddress ="宁夏回族自治区"
    RSarea ="银川市市场监督管理局兴庆二分局"
    RSFalseHeadlines ="宁夏回族自治区的银川市市场监督管理局兴庆二分局数据"
