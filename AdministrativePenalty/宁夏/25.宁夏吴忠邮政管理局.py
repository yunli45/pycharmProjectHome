...
from 宁夏.工具包 import 链接数据库, 替换标签, 附件下载程序,_25全文内容在表格中但是内容正常显示
...
class Utils(object):
    ...
    def parsePage(self, url=None, pageNo=None, baseUrl=None, SavePath=None,RSaddress=None,RSarea=None,RSFalseHeadlines=None):
        ...
        responseList = responseList.find_all('div',attrs={'class':'news-blocks'})
        # print(responseList)
        RSlist = re.findall(re.compile(r"<h3><a.*?href='(.*?)'.*?><!--(.*?)-->.*?</a></h3><em>(.*?)</em>", re.S | re.M),str(response))
        # print(RSlist)
        # print(len(RSlist))
        SrcList = []
        TitleList = []
        ...
        for src in SrcList:
            ...
            RSTime = TimeList[SrcList.index(src)].replace("&nbsp;",'').replace("[",'').replace("]",'')
            RSdataId = re.sub(r'.*?/', '', src).replace('.html', '')
            RSdataId = re.sub(r'.*?openId','',RSdataId)
            ...
            ConetentResponseSoupOld = ConetentResponseSoup.find('form', attrs={'name': 'form1'})
            ...
            OnlyID = self.OnlyID
            header = self.header
            fields = _25全文内容在表格中但是内容正常显示.getNeed(ConetentResponseSoupOld)
            """"
            表中的字段 公共部分插入到数据库
            """
            ...
            # 假如存在数据，则保存数据道数据库
            if ConetentResponse:
                sql1 = " INSERT INTO  AdministrativePunNingXia.dbo.crawlData20(dataId,title,documentNum,bePunished,principal,lawEnforcement,punishedDate,content,uniqueSign,address,area,agency,grade,showId,FalseHeadlines,Page,sourceUrl) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s') " % (
...
if __name__ =="__main__":
    # 共计1页数据
    url ="http://xzzf.spb.gov.cn/SPXzzfApp/base/spBaseXzzfOpenAction_cityOf!toSpXzzfOpenSC.action?infoTypeCode=3&undertakeUnitCode=100100070005"
    baseUrl="http://xzzf.spb.gov.cn/SPXzzfApp/base/"
    SavePath = "F:\行政处罚数据\宁夏\吴忠市\%s"
    RSaddress ="宁夏回族自治区"
    RSarea ="吴忠市" # 在全文的地址栏去匹配
    RSFalseHeadlines ="宁夏回族自治区宁夏吴忠邮政管理局的数据"
