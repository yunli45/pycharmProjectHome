...
class Utils(object):
    ...
    def disposeFormat1 (self,conent):
        ...
        处理中的全文 = re.sub(r'<span.*?>', '', 处理中的全文).replace('</span>', '')
        处理中的全文 = re.sub(r'<a.*?>', '', 处理中的全文).replace('</a>', '')
        处理中的全文 = re.sub(r'<td.*?>', '<p>', 处理中的全文).replace('</td>', '</p>')
        处理中的全文 = re.sub(r'<tr.*?>', ' ', 处理中的全文).replace('</tr>', ' ')
        处理中的全文 = re.sub(r'<p.*?>', '<p>', 处理中的全文).replace('</tr>', ' ')
        处理中的全文 = re.sub("<style.*?>.*?</style>",'',处理中的全文)
        处理中的全文 = re.sub(r'<strong.*?>', '', 处理中的全文).replace('</strong>', '').replace('</u>', '').replace('<u>','').replace("<o:p>", '').replace("</o:p>", '')
        return  处理中的全文
    # 替换文中的中文时间为阿拉伯数字格式
