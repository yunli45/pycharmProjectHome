    ...
    def parsePage(self,url,pageNo,baseUrl,baseUrl2,Savepath):
        ...
        for src in srcList:
            ...
            if RScontent ==None:
                ...
                RSsourceModel = "来源中华人民共和国生态环境部==》科技标准司==》环境保护标准==》标准文本==》 其他环境保护标准==》污染防治技术政策"  # 来源网址的那个模块
            print(ContentSrc)
            ...
            if RScontent:
                ...
                if not cur:
                    ...
                    print("数据库链接成功")
                sql1 = " INSERT INTO  environmentalStandardsTextForPollutionPreventionJSZC (id,title,time,sourceModel,requestUrl,sourceUrl,sourcePage,content,insertTime) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s') " % (RSid,RStitle,RStime,RSsourceModel,RSrequestUrl,RSsourceUrl,RSsourcePage,RScontent,RSinsertTime)
                print(sql1)
                if adjunct:
...
if __name__ =="__main__":
    # 共计353页
    url ="http://kjs.mee.gov.cn/hjbhbz/bzwb/wrfzjszc/index"
    baseUrl ="http://kjs.mee.gov.cn/hjbhbz/bzwb/wrfzjszc/"
    baseUrl2 =""
    Savepath ="F:\9.1环境生态部\第二个网址-标准文本\污染防治技术政策\%s"
