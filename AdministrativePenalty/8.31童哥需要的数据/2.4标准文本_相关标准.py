        ...
        for src in srcList:
            ...
            if RScontent:
                ...
                if not cur:
                    ...
                    print("数据库链接成功")
                sql1 = " INSERT INTO  environmentalStandardsTextForWaterXGBZ(id,title,time,sourceModel,requestUrl,sourceUrl,sourcePage,content,insertTime) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s') " % (RSid,RStitle,RStime,RSsourceModel,RSrequestUrl,RSsourceUrl,RSsourcePage,RScontent,RSinsertTime)
                print(sql1)
                if adjunct:
...
if __name__ =="__main__":
    # 共计353页
    url ="http://kjs.mee.gov.cn/hjbhbz/bzwb/shjbh/xgbzh/index"
    baseUrl ="http://kjs.mee.gov.cn/hjbhbz/bzwb/shjbh/xgbzh/"
    baseUrl2 =" "
    Savepath ="F:\9.1环境生态部\第二个网址-标准文本\水环境\相关标准\%s"
