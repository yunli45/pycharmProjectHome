...
class Utils(object):
    ...
    def parsePage(self,url,pageNo,baseUrl1,baseUrl2,Savepath):
        ...
        for src in srcList:
            ...
            RSsourceModel = "来源中华人民共和国生态环境部==》 科技标准司==》 环境保护标准==》 标准发布"  # 来源网址的那个模块
            ...
            if RScontent:
                ...
                if not cur:
                    ...
                    print("数据库链接成功")
                sql1 = " INSERT INTO  environmentalStandardSolicitation(id,title,time,sourceModel,requestUrl,sourceUrl,sourcePage,content,insertTime) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s') " % (RSid,RStitle,RStime,RSsourceModel,RSrequestUrl,RSsourceUrl,RSsourcePage,RScontent,RSinsertTime)
                print(sql1)
                if adjunct:
...
if __name__ =="__main__":
    # 共计353页
    url ="http://kjs.mee.gov.cn/hjbhbz/bzzqyj/index"
    baseUrl1 ="http://www.mee.gov.cn/"
    baseUrl2 =" "
    Savepath ="F:\9.1环境生态部\第二个网址-标准发布\%s"
