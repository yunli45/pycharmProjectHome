...
class Utils(object):
    ...
    def getPage(self,url=None):
        ...
        return response
    def parsePage(self,url,pageNo,baseUrl,baseUrl2,baseUrl3,Savepath):
        ...
        for src in srcList:
            ...
            if status_code == 404:
                ...
                ConetentResponse2 = requests.get(ContentSrc, headers=self.header)
                status_code2 = ConetentResponse2.status_code
                if status_code2 == 404:
                    ContentSrc = baseUrl3 + src
                    ConetentResponse = self.getPage(ContentSrc)
                else:
                    ConetentResponse = self.getPage(ContentSrc)
            ...
            ConetentResponseSoupOld2 = ConetentResponseSoup.find('div', attrs={'id': 'Custom_UnionStyle'})
            ConetentResponseSoupOld3 = ConetentResponseSoup.find('div', attrs={'id': 'ContentRegion'})
            ...
            ConetentResponseSoupOld = str(ConetentResponseSoupOld).replace(r"2312'>", '').replace("</ul>",'').replace("<sub>",'').replace("<_sub>",'').replace("</sub>",'').replace("<sup>",'').replace("<_sup>",'').replace("</sup>",'')
            ...
            if RScontent ==None:
                ...
                RSsourceModel = "来源中华人民共和国生态环境部==》科技标准司==》环境保护标准==》标准文本==》 其他环境保护标准==》环境监测方法标准及监测规范"  # 来源网址的那个模块
            print(ContentSrc)
            ...
            if RScontent:
                ...
                if not cur:
                    ...
                    print("数据库链接成功")
                sql1 = " INSERT INTO  environmentalStandardsTextForEnvironmentalMonitoringFFGF(id,title,time,sourceModel,requestUrl,sourceUrl,sourcePage,content,insertTime) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s') " % (RSid,RStitle,RStime,RSsourceModel,RSrequestUrl,RSsourceUrl,RSsourcePage,RScontent,RSinsertTime)
                print(sql1)
                if adjunct:
...
if __name__ =="__main__":
    # 共计353页
    url ="http://kjs.mee.gov.cn/hjbhbz/bzwb/jcffbz/index"
    baseUrl ="http://kjs.mee.gov.cn/hjbhbz/bzwb/jcffbz/"
    baseUrl2 ="http://kjs.mee.gov.cn/hjbhbz/bzwb/dqhjbh/"
    baseUrl3 ="http://kjs.mee.gov.cn/hjbhbz/bzwb/"
    Savepath ="F:\9.1环境生态部\第二个网址-标准文本\环境监测方法标准及监测规范\%s"
    AdminiStrative =Utils()
    for i in range(24,33):
        AdminiStrative.parsePage(url,str(i),baseUrl,baseUrl2,baseUrl3,Savepath)
        time.sleep(2)
