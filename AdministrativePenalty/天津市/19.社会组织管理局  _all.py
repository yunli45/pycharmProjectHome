    ...
    def parsePage(self,url=None,pageNo=None,baseUrl="http://scjg.tj.gov.cn",path="F:\行政处罚数据\天津\市场和质量监督管理局_和平区\/%s"):
        ...
        for src in srcList:
            ...
            if str(ConetentResponseSoupOld).find("<table") != -1 and  1 <= ContentTrNum <= 4 and ContentPNum <= 3:
                ...
                if listTr1[-1]:  # 不等于0就会去表格的最后一行tr，方便取值
                    tds = tr.find_all('td')
            ...
            # 全文可能包含表格，也可能不包含
            elif  str(ConetentResponseSoupOld).find("<table") !=-1 :
                print("全文含有表格")
                ...
                # 全文是表格的形式，且不止4个tr
                if ContentTrNum >4 and  ContentPNum <=2:
                    listTr2 = []
                    for ids, tr in enumerate(ConetentResponseAndTabe.find_all('tr')):
                        ...
                        if TrDocNUM:
                            ...
                            RScontent = '特殊情况，查看你下'
                # 全文是2个或3个tr 加上 正文的多个P标签
                elif ContentTrNum !=0  and ContentPNum >=4:

                    RSbePunished = ''  # 被处罚人或机构
                    RSlawEnforcement = ''  # 被处罚单位
                    RSprincipal = ''  # 法定代表人
                    RSdocumentNum = ''  # 书文号
                    RSagency = '天津市和平区市场和质量监督管理局' # 作出行政处罚决定机关名称
                    RSpunishedDate =RSTime  # 受处罚时间
                    RScontent =  str(ConetentResponseTable)
            # 不含有表格
            else:
                # 处理掉空格
                print("这里是不含有表格的")
