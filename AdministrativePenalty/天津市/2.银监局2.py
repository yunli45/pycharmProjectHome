...
class Utils(object):
    ...
    def parsePage(self,url=None,pageNo=None,baseUrl="http://www.cbrc.gov.cn",path="F:\行政处罚数据\天津\滨海新区\/%s"):
        ...
        for src in srcList:
            ...
            rs3 = re.sub('<span.*?>', '', rs2, flags=re.M | re.S).replace('</span>', '').replace('\xa0','')
            rs3 = re.sub('<p.*?class="MsoNormal".*?><o:p> </o:p></p>', '', rs3).replace('<o:p></o:p>', '')
            rs3 = re.sub(r'<st1:chsdate.*?>', '', rs3, flags=re.S).replace('</st1:chsdate>', '').replace('<o:p>','').replace('</o:p>','')
            rs3 = re.sub(r'<b.*?>','', rs3, flags=re.S)
            ...
            punishedDate = rsTmiePP[0] # 受处罚时间
            content = MsoNormal[12]+":"+MsoNormal[13]+"。"+MsoNormal[14]+":"+MsoNormal[15]+"。"+MsoNormal[16]+":"+MsoNormal[17]
