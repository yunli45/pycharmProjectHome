...
class Utils(object):
    ...
    def parsePage(self,url=None,pageNo=None,baseUrl=" ",path="F:\行政处罚数据\天津\环保局--红桥区\%s"):
        if pageNo =="1":
            response = self.getPage("http://hbj.tjhqqzf.gov.cn/zcfg/index.shtml")
            # print(url+".html")
        else:
            if int(pageNo)< 9:
                response = self.getPage(url + str(18 - int(pageNo)) + ".shtml")
            else:
                response = self.getPage(url + "0" + str(18 - int(pageNo)) + ".shtml")
        ...
        # homePageSoup = homePageSoup.findAll('table',attrs={'border':'0'})
        # print("这也是mmp")
        # print(homePageSoup)
        srcListFind = re.findall(re.compile(r'<td>・<a href="(.*?)".*?title="(.*?)">.*?</a></td>.*?<td width="25%">(.*?)</td>', re.S),str(response))
        # print("这是mmp")
        # print(srcListFind)
        # print("\n\n")
        titleList=[]
        ...
        for src in srcList:
            ...
            ConetentResponseSoupOld = ConetentResponseSoup.find('td', attrs={'id': 'fontzoom'})
            ...
            if RSTitle.find("天津市红桥区环境保护局行政处罚决定书津红环罚字") !=-1:
                ConetentNOW = BeautifulSoup(ConetentResponse,'lxml')
                ConetentNOWp = ConetentNOW.find_all('p')
                for p in ConetentNOWp:
                    # print(p)/
                    rs = re.findall(re.compile(r'>津红环罚字.*?\d号'),str(p))
                    if rs:
                        ...
                        RSprincipal = ConetentNOWp[num + 4].text.strip()  # 法定代表人
                        RSdocumentNum = ConetentNOWp[num].text.strip()  # 书文号
                        RSagency = '天津市红桥区环境保护局'  # 作出行政处罚决定机关名称
...
if __name__ =="__main__":
    # 共计353页
    url ="http://hbj.tjhqqzf.gov.cn/system/more/18040000/0000/18040000_000000"
