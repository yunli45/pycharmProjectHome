...
class Utils(object):
    ...
    def parsePage(self,url=None,pageNo=None,baseUrl="http://scjg.tj.gov.cn/baodi",path="F:\行政处罚数据\天津\市场和质量监督管理局_宝坻区\%s"):
        if pageNo =="1":
        ...
        for src in srcList:
            ...
            if src.find("http")!=-1:
                ...
                src = src.replace('../../','/')
                ContentSrc = "http://scjg.tj.gov.cn/baodi"+src
            print(ContentSrc)
            # print(ContentSrc)
            ...
            if str(ConetentResponseSoupOld).find("<table") != -1 and  1 <= ContentTrNum <= 13 and ContentPNum < 60 and str(ConetentResponseSoupOld).find("行政处罚信息摘要")==-1:
                ...
                for ids, tr in enumerate(TableSoup.find_all('tr')):
                    listTr1.append(ids)
                    TrDocNUM = re.findall(re.compile(r'<td>津[市 场 罚].*?\d.*?号.*?</td>'), str(tr))
...
if __name__ =="__main__":
    # 共计353页
    url ="http://scjg.tj.gov.cn/baodi/zwgk/xzcfxx/index"
