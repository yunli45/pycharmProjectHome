...
class Utils(object):
    ...
    def getPage(self,url=None):
        ...
        return response
    def parsePage(self,url=None,pageNo=None,baseUrl="http://ly.tj.gov.cn",path="F:\行政处罚数据\天津\旅游局通知公告\/%s"):

        if pageNo =="1":
            response = self.getPage(url+".htm")
        else:
            response = self.getPage(url+"_"+str(int(pageNo)-1)+".htm")
        print("+++++++++++++++++++这是第："+pageNo+"页++++++++++++++++")
        # print(response)

        homePageSoup = BeautifulSoup(response,'lxml')
        homePageSoup = homePageSoup.findAll('div',attrs={'class':'cnl'})
        # print("这也是mmp")
        print(homePageSoup)
        srcListFind = re.findall(re.compile(r'<a href="(.*?)".*?>(.*?)</a><span>(.*?)</span>', re.S),str(homePageSoup))
        # print(srcListFind)
        titleList=[]
        timeList=[]
        srcList =[]
        ...
        for src in srcList:
            ...
            print(ContentSrc)
            # print(ContentSrc)
            ConetentResponse = request.Request(ContentSrc,headers = self.headers)
            ConetentResponse = request.urlopen(ConetentResponse)
            ConetentResponse = ConetentResponse.read().decode()

            ConetentResponseSoup = BeautifulSoup(ConetentResponse, 'lxml')
            ConetentResponseSoupOld = ConetentResponseSoup.find('div', attrs={'class': 'nc'})
            ...
            # 不含有表格
                ...
                if RSimgFind:
                    ...
                    i = 1
                    for srcImg in RSimgFind:
                        imgSrc = srcImg
                        imgName =RSTitle.replace("”","】").replace("“","【")+str(i)+".jpg"
                        # print("图片名字"+str(imgSrc1))
                        path1 = "F:\行政处罚数据\天津\旅游局通知公告\/%s" %(imgName)
                        # print(path1)
                        try:
                            request.urlretrieve(imgSrc,path1)
                        except:
                            pass
                        i+=1
                else:
            ...
            if ConetentResponse:
                ...
                if adjunct:
                    ...
                    for xiaZai in adjunct:
                        # print(xiaZai)
                        rsDocuniqueSign = str(xiaZai[0])
                        rsDocName = str(xiaZai[1])
                        xiaZai = str(xiaZai)
                        ...
                        if rsDoc1 or rsDoc2:
                            ...
                            rsDocName = rsDocName.replace("/", '_').replace('<u>','').replace('</u>','').replace('<','').replace('>','')
                            path1 = path % (rsDocName)
                            if rsDocuniqueSign.find("http") == -1:
                                rsDocuniqueSign =str( "%s" % (baseUrl) + rsDocuniqueSign)
                            else:
                                rsDocuniqueSign = str(rsDocuniqueSign)
                            try:
                                request.urlretrieve(rsDocuniqueSign,path1)
                            except :
                                pass
                            ...
                            rsDocName = rsDocName.replace("/", '_').replace('<u>', '').replace('</u>','').replace('<',
                                                                                                               '').replace(
                                '>', '')
                            path1 = path % (rsDocName)
                            if rsDocuniqueSign.find("http") == -1:
...
if __name__ =="__main__":
    ...
    # 共计353页
    url ="http://ly.tj.gov.cn/Class/tongzhigonggao/index"
