...
class Utils(object):
    ...
    def parsePage(self,url=None,pageNo=None,baseUrl="http://www.tjwq.gov.cn",path="F:\行政处罚数据\天津\市场和质量监督管理局_武清区\%s"):
        if pageNo =="1":
            response = self.getPage(url+".shtml")
            print(url+".html")
        else:
            response = self.getPage(url+"_"+str(int(pageNo)-1)+".shtml")

        print("+++++++++++++++++++这是第："+pageNo+"页++++++++++++++++")
        # print(response)

        homePageSoup = BeautifulSoup(response,'lxml')
        homePageSoup = homePageSoup.findAll('div',attrs={'class':'left listright'})
        # print("这也是mmp")
        # print(homePageSoup)
        srcListFind = re.findall(re.compile(r'<span.*?>(.*?)</span>.*?<a href="(.*?)".*?title="(.*?)">.*?</a>', re.S),str(homePageSoup))
        # print("这是mmp")
        # print(srcListFind)
        titleList=[]
        timeList=[]
        ...
        for src in srcList:
            ...
            RSdataId = re.sub(r'.*?/', '', src).replace('.shtml', '')

            if src.find("http")!=-1:
                ContentSrc = src
            else:
                src = src.replace('../../','/')
                ContentSrc = "http://www.tjwq.gov.cn"+src
            print(ContentSrc)
            # print(ContentSrc)
            ...
            ConetentResponseSoupOld = ConetentResponseSoup.find('div', attrs={'id': 'zoom'})
            ...
            if str(ConetentResponseSoupOld).find("<table") != -1 and  1 <= ContentTrNum <= 13 and ContentPNum < 60 and str(ConetentResponseSoupOld).find("政处罚信息摘要")==-1:
                # 先去除多余的标签
                ...
                rss = re.findall(re.compile(r'<p>营业执照(.*?)</p>'),ConetentResponse)
                print("营业执照注册号"+str(rss))
                if  rss!=[]:
                    ...
                    for p in ConetentNOWp:
                        RSdocumentNumFind = re.findall(re.compile(r'津武.*?号'), str(p))
                        AdressNumFind = re.findall(re.compile(r'地址.*?'), str(p))
                        print("书文号"+str(RSdocumentNumFind))
            ...
            # 不含有表格
                ...
                ConetentNoTable  = ConetentResponse.replace(' ','').replace("当事人名称或姓名：",'').replace("法定代表人（负责人）：",'').replace("法定代表人：",'')
                # 全文还可能是一个图片的形式
                RSimgFind = re.findall(re.compile(r'<img.*?src="(.*?)".*?>'), ConetentResponse)
                ...
                for p in ConetentNOWp:
                    ...
                    if RSdocumentNumFind!=[]:
                        documentPNum =  ConetentNOWp.index(p)
                        RSdocumentNum = ConetentNOWp[documentPNum].text.strip()  # 书文号

                    if AdressNumFind != []:
                        print("AdressNumFin地址存在的情况下")
                        print(p)
                        AdressNum =  ConetentNOWp.index(p)
                        print("hajhahah ")
                        print(AdressNum)
...
if __name__ =="__main__":
    # 共计353页
    url ="http://www.tjwq.gov.cn/hbj/zwwjian/list"
    AdminiStrative =Utils()
    for i in range(117,139):
