...
class Utils(object):
    ...
    def parsePage(self,url=None,pageNo=None,baseUrl="http://scjg.tj.gov.cn/jgdt/xzcfxxgs/",path="F:\行政处罚数据\天津\海事局\%s"):
        if pageNo =="1":
            response = self.getPage(url+".jhtml")
            print(url+".html")
        else:
            response = self.getPage(url+"_"+str(int(pageNo)-1)+".jhtml")
        ...
        for src in srcList:
            ...
            if str(ConetentResponseSoupOld).find("<table") != -1 and  1 <= ContentTrNum <= 13 and ContentPNum < 110 and str(ConetentResponseSoupOld).find("政处罚信息摘要")==-1:
                # 先去除多余的标签
