...
class Utils(object):
    ...
    def parsePage(self,url=None,pageNo=None,baseUrl=" ",path="F:\行政处罚数据\天津\市场和质量监督管理局 _北辰区\%s"):
        if pageNo =="1":
        ...
        for src in srcList:
            ...
            ConetentResponse = re.sub(r'<p.*?>', '<p>', str(ConetentResponse))
            ...
            # 全文可能包含表格
            elif  str(ConetentResponseSoupOld).find("<table") != -1 and str(ConetentResponseSoupOld).find("政处罚信息摘要")!=-1:
                print("全文含有表格")
                ...
                ConetentResponseTable = re.sub(r'<p.*?>','',ConetentResponseTable).replace('</p>','')  #  去除含有表格的时候 p标签包含这td
...
if __name__ =="__main__":
    # 共计353页
    url ="http://scjg.tj.gov.cn/beichen/zwgk/xzcfxx/index"
