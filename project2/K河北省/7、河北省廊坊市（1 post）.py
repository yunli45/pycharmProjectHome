                    ...
                    for ids, src in enumerate(src_list):
                        ...
                        if data_id_rs is not None:
                            ...
                            if cont_soup_fj != []:
                                ...
                                print("这篇文章有附件"+str(title))
                                print(cont_fj)
                                with open(r'D:\Python\PyCharm 20181.4\project\project1\K河北省\jl.txt', 'a') as f:
                                    f.write(page_no_position+"/n")
                            else:
                                cont_fj = " "

                            # page_url 现在用不着了
                            cont_cont_fj = 预处理模块.dispose_of_data(index_url, '', content_src, str(cont_fj), save_path, annex_local)
...
if __name__ == "__main__":
    ...
    table_name = '行政案例数据表'
    # table_name = '行政案例测试表'
    save_path = "E:\行政案例附件\datafolder\河北省\廊坊市\所有行业"
    source_library = '河北省>廊坊市>所有行业'
