...
class MySpider():
    ...
    def parse_page(self):
        ...
        rs = soup.find_all('table', attrs={'class':'listmain'})
        for i in rs:
            tr = i.find_all('tr')
            rs_1 = tr[1:]
        rs_2 = str(rs_1).replace("\n",'').replace('\t', '').replace('\r', '').replace("'", '').replace(" ", '')
        rs_list = re.findall(r"""<tr><td.*?></td><td.*?></td><td><ahref="(.*?)".*?>(.*?)</a></td><td.*?></td><td.*?>(.*?)</td>""", str(rs_2))
        """
            # 这一组市用于解析json 
            ...
            if src_list:
                for ids, src in enumerate(src_list):
                    ...
                    if data_id_rs is not None:
                        ...
                        rs_cont_soup = rs_cont_soup.find_all('div', attrs={'id':'artibody'})
...
def main():
    ...
    for page_no in range(1, 2):
        para = ''
        page_url = index_url + "/channels/458.shtml"
        request_type = 'get'
