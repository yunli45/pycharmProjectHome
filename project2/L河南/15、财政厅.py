...
class MySpider():
    ...
    def parse_page(self):
        ...
        rs = soup.find_all('ul', attrs={'id':'newslist'})
        for i in rs:
            li = i.find_all('li')

        rs_2 = str(li).replace("\n",'').replace('\t', '').replace('\r', '').replace("'", '')
        rs_list = re.findall(r"""<li><a.*?href="(.*?)".*?>(.*?)</a><span>(.*?)</span></li>""", str(rs_2))
        """
            # 这一组市用于解析json 
        ...
        if rs_list:
            ...
            if src_list:
                for ids, src in enumerate(src_list):
                    ...
                    book_num = title.replace('河南省财政厅行政处罚决定书', '').replace("(", '').replace("（", '').replace("）", '').replace(")", '')
                    ...
                    if data_id_rs is not None:
                        ...
                        rs_cont_soup = rs_cont_soup.find_all('div', attrs={'class':'newstxt'})
...
def main():
    ...
    index_url = "http://www.hncz.gov.cn"
    table_name = '行政案例数据表'
    # table_name = '行政案例测试表'
    source_library = '河南省>财政厅'
