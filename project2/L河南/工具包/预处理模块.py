...
import re
import requests
from L河南.工具包 import 链接数据库, 附件下载程序, 判断url前面的点返回完整的请求地址
...
def dispose_of_data(indexUrl, page_url, content_src, content, save_path, module_name):
    ...
    #     for a in all_a:
    #         # 循环所有的a标签,再遍历每一个a标签，在每一个a标签中去匹配是否是附件形式 ，不是就直接替换掉，是的话在进行下一步：匹配附件a标签中冲链接和文本能容
    #         annex_a = re.findall(r'<a.*?href="(.*?)".*?>(.*?)</a>', a)
    #         if annex_a !=[]:
    #             print("开始补全超链接的地址..................")
    #             a_hyper = annex_a[0][0]
    #
    #             annex_download_url = 判断url前面的点返回完整的请求地址.returnSRC().returnSrc(indexUrl, a_hyper, content_src)
    #             annex_head = requests.get(annex_download_url).headers
    #             adj_type = annex_head['Content-Type']
    #             adj_hz = re.findall('.*?(pdf|docx|doc|xlsx|xls|rar|zip|jpeg|jpg|png|gif|txt|7z|gz|shtml|html)', str(adj_type))
    #             adj_hz = re.findall('.*?(excel|gif|html)', str(adj_type))
    #
    #             if adj_hz:
    #                 print(adj_hz)
    #                 # 当结果不是网页那就添加后缀，是网页的话就将网页的链接去掉
    #                 if adj_hz[0] != "html":
    #                     pass
    #                 else:
    #                     print("这个a标签不是附件，是跳转" + str(a) + "\n" + "\n")
    ...
    #                     content = content.replace(before_a, new_a1)
    #             else:
    #                 raise  Exception("匹配出的结果不在已知的类型中：excel 、 html、gif,这次匹配出的结果为：{0}".format())
