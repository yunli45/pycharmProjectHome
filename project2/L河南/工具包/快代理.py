...
def checkip(targeturl, ip):
    ...
    try:
        resp = requests.get(url=targeturl, proxies=proxies, headers=headers, timeout=5)
        response_code = resp.status_code
        response = resp.content.decode('utf-8').replace(u'\xa0', u'')
...
def findip(type, pagenum, targeturl, path):  # ip类型,页码,目标url,存放ip的路径
    list = {'1': 'https://www.kuaidaili.com/free/inha/',  # 国内高匿代理
            '2': 'https://www.kuaidaili.com/free/intr/'   # 普通代理
            }
...
def getip(targeturl, path):
    ...
    threads = []
    for type in range(2):  # 四种类型ip,每种类型取前三页,共12条线程
