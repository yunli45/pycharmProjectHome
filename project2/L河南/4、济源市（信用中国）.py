    ...
    def parse_page(self):
        ...
        if rs_list:
            for i in rs_list:
                ...
                penalty_date_list.append(i[4]) # 注意这里的处罚时间和公示时间是一至的，所以后期将处罚时间集合使用，公示时间集合用于存储处罚的事项与处罚人拼接成为一个新的标题
            ...
            if src_list:
                for ids, src in enumerate(src_list):
                    ...
                    if data_id_rs is not None:
                        ...
                        cont_t =  提取信息.extracting_information_table_8(rs_cont)
                        # (book_number,legal_person, cont)
                        ...
                        area = ''
                        cont = cont_t[2]
                        penalty_date = penalty_date_list[ids]  # 处罚时间
...
def main():
    # index_url = "2"
    index_url = "http://credit.ruzhou.gov.cn"
    table_name = '行政案例数据表'
    # table_name = '行政案例测试表'
    source_library = '河南省>信用中国>汝州市'
    ...
    for page_no in range(1,296):
        para = ''
        page_url = index_url + "/creditsearch.permissionList.phtml?id=&keyword=&page={0}".format(page_no)
