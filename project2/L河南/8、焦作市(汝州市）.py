...
class MySpider():
    ...
    def parse_page(self):
        ...
        rs_list = re.findall(r'<td><a.*?href="(.*?)".*?>(.*?)</a></td><td><a.*?>(.*?)</a></td><td><a.*?>(.*?)</a></td><td><a.*?>(.*?)</a></td></tr>', str(rs), flags=re.M)
        # print(rs_list)
        # print(input())
        if rs_list:
            for i in rs_list:
            ...
            if src_list:
                for ids, src in enumerate(src_list):
                    ...
                    if data_id_rs is not None:
                        print("这条数据在已有的数据库中已存在，现在已经paa掉了，依据的原则是这条数据完整的请求地址,标题为： " )
                        pass
                    else:
                        ...
                        """
                            提取详细数据页面的相关信息：
                        """
                        rs_cont = self.get_page_get(content_src)
                        cont_t =  提取信息.extracting_information_table_9(rs_cont)
                        #  (title, book_number, punished_people, law_enforcement, legal_person, date, cont)
                        title = cont_t[0]
                        book_num = cont_t[1].replace('"', '')
                        if book_num == '空':
                            book_num =''
                        punished_people = cont_t[2]
                        punished_institution = ''
                        law_enforcement = cont_t[3]  # 执法机构
                        legal_person = cont_t[4]
                        area = ''
                        penalty_date = cont_t[5]  # 处罚时间
                        cont = cont_t[6]
...
def main():
    # index_url = "2"
    index_url = "http://creditjz.gov.cn"
    table_name = '行政案例数据表'
    # table_name = '行政案例测试表'
    source_library = '河南省>信用中国>焦作市'
    ...
    for page_no in range(1,20):
        para = ''
        page_url = index_url + "/creditsearch.punishmentList.phtml?id=&keyword=&page={0}".format(page_no)
        MySpider(headers, index_url, page_no, page_url, para, save_path, table_name, source_library, annex_local,
                 province, proxies).parse_page()
