    ...
    def parse_page(self):
        ...
        if rs_list:
            ...
            for ids,i in enumerate(rs_list):
                ...
                title_list.append(i['xyztmc'])
                tyshxydm_list.append(i['tyshxydm'])
                zzjgdm_list.append(i['zzjgdm'])
                gszch_list.append(i['gszch'])
                swdjh_list.append(i['swdjh'])
                mc_list.append(i['mc'])
                cflb1_list.append(i['cflb1'])
                cflb2_list.append(i['cflb2'])
                sy_list.append(i['sy'])
                yj_list.append(i['yj'])
                jg_list.append(i['jg'])
                law_enforcement_list.append(i['xzjg']) # 执法机关
                penalty_date_list.append(i['cfrq'])
                # release_date_list.append(i[3])  # 发布时间（公示时间）
                punished_people_list.append(i['xyztmc']) # 被处罚人
            ...
            if src_list:
                for ids, src in enumerate(src_list):
                    ...
                    content_src = self.index_url + '/xygsxq/?queryType=frcf&id=' + src
                    ...
                    if data_id_rs is not None:
                        ...
                        # cont_t =  提取信息.extracting_information_table_11(rs_cont)
                        ...
                        <table border="1" cellspacing="0" align="center">
                            <tr><td>信用主体名称</td><td>{0}</td></tr>
                            <tr><td>统一社会信用代码</td><td>{1}</td></tr>
                            <tr><td>组织机构代码</td><td>{2}</td></tr>
                            <tr><td>工商登记码</td><td>{3}</td></tr>
                            <tr><td>税务登记号</td><td>{4}</td></tr>
                            <tr><td>处罚文书号</td><td>{5}</td></tr>
                            <tr><td>案件名称</td><td>{6}</td></tr>
                            <tr><td>处罚类别1</td><td>{7}</td></tr>
                            <tr><td>处罚类别2</td><td>{8}</td></tr>
                            <tr><td>处罚事由</td><td>{9}</td></tr>
                            <tr><td>处罚依据</td><td>{10}</td></tr>
                            <tr><td>处罚结果</td><td>{11}</td></tr>
                            <tr><td>处罚决定日期</td><td>{12}</td></tr>
                            <tr><td>处罚机关</td><td>{13}</td></tr>
                           </table>
                        """.format(title,  tyshxydm_list[ids], zzjgdm_list[ids], gszch_list[ids], swdjh_list[ids], book_num, mc_list[ids], cflb1_list[ids], cflb2_list[ids], sy_list[ids], yj_list[ids], jg_list[ids], penalty_date, law_enforcement)
...
def main():
    ...
    index_url = "http://xyhb.hebi.gov.cn"
    table_name = '行政案例数据表'
    # table_name = '行政案例测试表'
    source_library = '河南省>信用中国>鹤壁市'
    ...
    for page_no in range(1, 90):
        para = ''
        page_url = index_url + "/selectSgsInfo.jspx?type=frcf&keyword=&pageSize=12&pageNo={0}&_=1553675199532".format(page_no)
        request_type = 'get'
