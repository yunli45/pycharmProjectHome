...
class MySpider():
    ...
    def parse_page(self):
        ...
        rs = soup.find_all('div', attrs={'class':'pcjg_fffbg'})
        rs = rs[1:10]
        rs = str(rs).replace("\n",'').replace('\t', '').replace('\r', '').replace("'", '').replace(" ", '')
        rs_list = re.findall(r"""<tr><td.*?><ahref="(.*?)".*?>(.*?)</a></td><td.*?><a.*?>(.*?)</a></td><td.*?>(.*?)</td><td.*?>(.*?)</td></tr>""", str(rs))
        """
            # 这一组市用于解析json 
        ...
        if rs_list:
            ...
            for ids,i in enumerate(rs_list):
                ...
                # release_date_list.append(i[''])  # 发布时间（公示时间）
            ...
            if src_list:
                for ids, src in enumerate(src_list):
                    ...
                    content_src = self.index_url + src.replace("&amp;", '&').replace("..", '')
                    ...
                    if data_id_rs is not None:
                        ...
                        legal_person = cont_t[0]          # 法人
                        penalty_date = cont_t[1]         # 处罚时间
                        cont = cont_t[2]
...
def main():
    ...
    proxies = {
        "HTTP": "http://222.135.92.68:38094",
        "HTTP": "http://117.87.177.239:	9000",
        "HTTP": "http://183.148.152.236:9999"
    }
    # 构造所有的url
    ...
    for page_no in range(1, 90):
        para = ''
        if page_no ==1:
            page_url = index_url + "/zhuanti/xzcf/getPcjgXZCFDocListDividePage/henan.html"
        else:
            page_url = index_url + "/zhuanti/xzcf/getPcjgXZCFDocListDividePage/henan.html?current={0}".format(page_no)
        request_type = 'get'
    ...
    #     page_url = index_url + '/publicity/ListPunishInfo.html'
    #     para = {
