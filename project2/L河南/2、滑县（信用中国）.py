...
class MySpider():
    ...
    def parse_page(self):
        ...
        rs = soup.find_all('tr', attrs={'class':'odd'})
        rs = str(rs).replace("\n",'').replace('\t', '').replace('\r', '')
        ...
        if rs_list:
            ...
            if src_list:
                for ids, src in enumerate(src_list):
                    ...
                    if title_1 == 'null':
                        title = punished_people_list[ids]
                    else:
                        title = title_1
                    title = title.replace('"', '')
...
def main():
    ...
    index_url = "http://www.hxcredit.gov.cn/"
    table_name = '行政案例数据表'
    # table_name = '行政案例测试表'
    source_library = '河南省>信用中国>滑县'
    ...
    for page_no in range(1, 447):
        para = ''
        page_url = index_url + "jcms/SearchPenalty.do?pageNum={0}&XDR=&XKNR=&organname=".format(page_no)
