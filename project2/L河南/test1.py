...
class myThread(threading.Thread):
    def __init__(self, headers, index_url,page_no, page_url, para, save_path, table_name, source_library, annex_local, province, q):
    ...
    def parse_page(self):
        ...
        response = self.get_page_data(self.page_url, self.para)
    ...
    # 访问网址:post
    def get_page_data(self, url, para):
        i = 0
        while i<3:
            try:
...
def main():
    ...
    # 构造所有的url
    url_list = []
    for page_no_1 in range(1, 20):
        para_1 = {
            "page": "{0}".format(page_no_1),
            "pagesize": "20"
        }

        url_list.append(para_1)
    page_url = index_url + "CMSInterface/cms/xzcflist"
    # 保存线程
    Thread_list = []

    # 创建线程并启动线程
    for ids,src in enumerate(url_list):
        page_no = ids + 1
        para = src

        my_thread = myThread(headers,index_url, page_no, page_url, para, save_path, table_name, source_library, annex_local, province,  q)
        my_thread.start()
