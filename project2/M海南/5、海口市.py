...
class MySpider():
    ...
    def parse_page(self):
        ...
        rs = soup.find_all('div', attrs={'class':'row'})
        # for i in rs:
        #     tr = i.find_all('tr')

        rs_2 = str(rs).replace("\n",'').replace('\t', '').replace('\r', '').replace("'", '').replace("(",'（').replace(")",'）')
        rs_list = re.findall(r"""<div class="row"><li.*?>.*?</li><li class="mc"><div><a href="(.*?)".*?>(.*?)</a></div></li><li class="sep".*?></li><li class="fbrq".*?>(.*?)</li><li class="sep".*?></li><li class="wh".*?>.*?</li>.*?</div>""", str(rs_2))
        """
            # 这一组市用于解析json 
        ...
        if rs_list:
            ...
            if src_list:
                for ids, src in enumerate(src_list):
                    ...
                    index_url = self.index_url + '/pub/root9'
                    src = src.replace("../../", '/')
                    ...
                    if data_id_rs is not None:
                        ...
                        head_b = 'class'
                        head_c = 'headInfo'
                        rs_head = rs_cont_soup.find_all('{0}', attrs={'{1}':'{2}'}).format(table_a, table_b, table_c)
...
def main():
    ...
    index_url = "http://www.haikou.gov.cn"
    # table_name = '行政案例数据表'
    table_name = '行政案例测试表'
    source_library = '海南省>海口市'
    ...
    for page_no in range(1, 46):
        ...
        # page_url = index_url + "/pub/root9/216/675/index_{0}.htm".format(page_no-1)
        request_type = 'get'
