...
class MySpider():
    ...
    def parse_page(self):
        ...
        rs = soup.find_all('table', attrs={'id':'documentContainer'})
        # for i in rs:
        #     tr = i.find_all('tr')

        rs_2 = str(rs).replace("\n",'').replace('\t', '').replace('\r', '').replace("'", '').replace("(",'（').replace(")",'）')
        rs_list = re.findall(r"""<tr><td.*?>.*?</td><td.*?><a.*?href="(.*?)".*?>(.*?)</a></td><td.*?>(.*?)</td><td.*?>(.*?)</td><td.*?>(.*?)</td></tr>""", str(rs_2))
        """
            # 这一组市用于解析json 
            ...
            if src_list:
                for ids, src in enumerate(src_list):
                    ...
                    if data_id_rs is not None:
                        ...
                        rs_cont_soup = rs_cont_soup.find_all('table', attrs={'width':'95%'})
...
def main():
    ...
    for page_no in range(1, 18):
        ...
        page_url = index_url + "/was5/search/search_hnpage.jsp?subcat1=&subcat2=&name2=&Publisher=wzsxxgk&PreKeyword=行政处罚&dengyu1=1&Referer=&siteid=&type=a&page={0}".format(page_no)
        request_type = 'get'
