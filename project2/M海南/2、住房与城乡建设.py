...
class MySpider():
    ...
    def parse_page(self):
        ...
        rs = soup.find_all('table', attrs={'id':'xxtaba'})
        # for i in rs:
        #     tr = i.find_all('tr')

        rs_2 = str(rs).replace("\n",'').replace('\t', '').replace('\r', '').replace("'", '').replace("(",'（').replace(")",'）')
        rs_list = re.findall(r"""<td colspan="2"><font class="font-b black">标　　题：</font><a href="(.*?)".*?>(.*?)</a></td></tr><tr><td><font.*?>发文字号：</font></td><td><font.*?>发布日期：</font>(.*?)</td></tr><tr>""", str(rs_2))
        """
            # 这一组市用于解析json 
            ...
            if src_list:
                for ids, src in enumerate(src_list):
                    ...
                    if data_id_rs is not None:
                        ...
                        rs_cont_soup = rs_cont_soup.find_all('div', attrs={'id':'zoomcon'})
...
def main():
    ...
    index_url = "http://zjt.hainan.gov.cn"
    table_name = '行政案例数据表'
    # table_name = '行政案例测试表'
    source_library = '海南省>住房与城乡建设'
