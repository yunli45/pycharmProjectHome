...
class MySpider():
    ...
    def parse_page(self):
        ...
        rs = soup.find_all('table', attrs={'id':'testUI'})
        # for i in rs:
        #     tr = i.find_all('tr')

        rs_2 = str(rs).replace("\n",'').replace('\t', '').replace('\r', '').replace("'", '').replace("(",'（').replace(")",'）')
        rs_list = re.findall(r"""<td.*?>.*?<a.*?href="(.*?)".*?title="(.*?)">.*?</a></td><td.*?>(.*?)</td>""", str(rs_2))
        """
            # 这一组市用于解析json 
        ...
        if rs_list:
            ...
            for ids,i in enumerate(rs_list):
                ...
                # law_enforcement_list.append(i[4]) # 执法机关
            ...
            if src_list:
                for ids, src in enumerate(src_list):
                    ...
                    if title.find("处罚信息公开表")!=-1:
                        book_num = title.replace('行政处罚决定书', '').replace("（", '').replace("）", '')
                    elif title.find("行政处罚决定书")!=-1:
                        book_num = title[title.find("行政处罚决定书")+7:]
                    else:
                        raise Exception("错误了,在本网址中暂时只找到两种规则：处罚信息公开表、行政处罚决定书，发现一种新的规则，请查看下这是第{0}页的数据，标题是{1}".format(self.page_no, title))
                    ...
                    if data_id_rs is not None:
                        ...
                        rs_cont_soup = rs_cont_soup.find_all('div', attrs={'class':'Section1'})
                        ...
                        book_num = book_num
                        punished_people = ''    # 被处罚人
                        punished_institution = ''        # 被处罚机构
                        law_enforcement = '河南省财政厅'      # 执法机构
                        legal_person = ''         # 法人
                        penalty_date =  release_date_list[ids]         # 处罚时间
                        cont = cont
...
def main():
    ...
    index_url = "http://www.hitax.gov.cn"
    # table_name = '行政案例数据表'
    table_name = '行政案例测试表'
    source_library_list = ['海口市', '三亚市', '洋浦区', '三沙市']
    url_list = ['/sxpd_1_5/index.html', '/sxpd_2_3/', '/sxpd_3_3/', '/sxpd_20_3/']
    source_library = '海南省>银监局'
    ...
    for page_no in range(1, 4):
        para = ''
        # if page_no ==1 :
        #     page_url = index_url + "/szjt/1600/right.shtml"
        #     print(page_url)
        # else:
        #     page_url = index_url + "/szjt/1600/right_{0}.shtml".format(page_no)
        page_url = index_url + "/chinese/home/docViewPage/60180406&current={0}".format(page_no)
        request_type = 'get'
