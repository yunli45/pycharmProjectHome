                ...
                for ids, src in enumerate(src_list):
                    ...
                    if data_id_rs is not None:
                        ...
                        if str(title).find('编号')!=-1:
                            rr = re.findall(r'.*?关于(.*?)侵犯.*?编号[: ：](.*?)）', str(title))
                        else:
...
def main():
    ...
    for page_no in range(1, 8):
        ...
        page_url = index_url + "/beijing_customs/434756/434811/434813/434814/39d89d58-{0}.html".format(page_no)
        request_type = 'get'
