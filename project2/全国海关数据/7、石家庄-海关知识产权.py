                ...
                for ids, src in enumerate(src_list):
                    ...
                    if data_id_rs is not None:
                        ...
                        if str(title).find('编号')!=-1:
                            rr = re.findall(r'(.*?)侵犯.*?编号[: ：](.*)', str(title_1))
                        else:
...
def main():
    ...
    index_url = "http://shijiazhuang.customs.gov.cn"
    table_name = '行政案例数据表'
    # table_name = '行政案例测试表'
    source_library = '石家庄>海关>海关知识产权'
    ...
    for page_no in range(1, 3):
        ...
        page_url = index_url + "/shijiazhuang_customs/456977/457003/457005/457006/fdb1d2e0-{0}.html".format(page_no)
        request_type = 'get'
