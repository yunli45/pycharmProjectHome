            ...
            if src_list:
                for ids, src in enumerate(src_list):
                    ...
                    if data_id_rs is not None:
                        ...
                        print(title)
                        title_1 = title.replace("（", '').replace("）", '')
                        if title_1.find("违规") != -1:
                            punished_people = title_1[0:title_1.find("违规")]
                            law_enforcement = ''
                            book_num = title_1[title_1.find("行政处罚决定书") + 7:]

                        else:
                            law_enforcement = title_1[:title.find("行政处罚决定书")]
                            book_num = title_1[title_1.find("行政处罚决定书") + 7:]
                            punished_people =''

                        punished_people = punished_people             # 被处罚人
                        punished_institution = ''                     # 被处罚机构
                        law_enforcement =  law_enforcement            # 执法机构
                        legal_person = ''                             # 法人
...
def main():
    ...
    index_url = "http://beijing.customs.gov.cn"
    # table_name = '行政案例数据表'
    table_name = '行政案例测试表'
    source_library = '北京市>海关>海关走私违规'
    ...
    for page_no in range(1, 34):
        ...
        #  beijing_customs/434756/434811/434813/434815/c5d10aed-1.html
        page_url = index_url + "/eportal/ui?pageId=434815&currentPage={0}&moduleId=c5d10aed655847f699ca4aea257bc568&staticRequest=yes".format(page_no)
        request_type = 'get'
