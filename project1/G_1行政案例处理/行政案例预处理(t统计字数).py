...
def check_hyper_link(excel_file_path):
    ...
    for row in range(2, rows+1):
        ...
        if 0 < len(now_cont) < 30000:
            ...
            char_num = len(re.sub('</？[^>]*>', '', str(now_cont)))
            table['O%s' % (row)] = char_num
...
def word_count(excel_file_path):
    data_table_rows = open_excel_file(excel_file_path)
    data = data_table_rows[0]
    table = data_table_rows[1]
    rows = data_table_rows[2]
    print(rows)
    for row in range(2, rows+1):
        content = table['K%s' % (row)].value
        char_num = len(re.sub('</？[^>]*>', '', str(content)))
        table['O%s' % (row)] = char_num
    data.save(excel_file_path)


# file_path = 'E:\环保局\环保局汇总\环保局'
excel_file_path = 'D:\Python\PyCharm 20181.4\project\project1\E.云南、上海预处理\【原始数据】云南省行政处罚案例数据.xlsx'
