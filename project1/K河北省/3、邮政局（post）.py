...
# 访问网址：get
    ...
    def pare_page_data(self, page_url, table_name, save_path, source_library, province, annex_local):
        ...
        for page_no in range(1, 11):
            ...
            rs_soup = rs_soup.find_all('div', attrs={'class': 'news-blocks'})
            rs_soup = str(rs_soup[0]).replace("\n", '').replace("\r", '').replace("\t"
                                                                                  "", '').replace('\xa0','')
            rs_list = re.findall("""<div class="news-block-tags"><h3><a.*?onclick="window.location.href='(.*?)' "><!--(.*?)--><script type="text/javascript">.*?</script></a></h3><em>\[(.*?)]</em></div>""", str(rs_soup))
            if rs_list:
                for i in rs_list:
                    src_list.append(i[0])
                ...
                if src_list:
                    for ids, src in enumerate(src_list):
                        ...
                        content_src = "http://xzzf.spb.gov.cn/SPXzzfApp/base/"+ src
...
if __name__ == "__main__":
    ...
    index_url = "http://xzzf.spb.gov.cn/SPXzzfApp/base/spBaseXzzfOpenAction_provinceOf!toSpXzzfOpenSC.action?infoTypeCode=3&undertakeUnitCode=10010044"  #
    table_name = '行政案例数据表'
    # table_name = '行政案例测试表'
    save_path = "E:\行政案例附件\datafolder\河北省\邮政局"
    source_library = '河北省>邮政局'
