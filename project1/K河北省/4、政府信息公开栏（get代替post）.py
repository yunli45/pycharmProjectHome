...
# 访问网址：get
    ...
    def pare_page_data(self, page_url, table_name, save_path, source_library, province, annex_local):
        ...
        for page_no in range(1, 31):
            src_list, date_list, title_list, book_num_list, law_enforcement_list = [], [], [], [], []
            # if page_no == 1:
            #     page_url_1 = page_url
            # else:
            ...
            rs_soup = rs_soup.find_all('table', attrs={'align':'center'})
            ...
            if rs_list:
                for i in rs_list:
                    ...
                    date_list.append(i[3])   # 发布时间（公示时间）
                if src_list:
                    for ids, src in enumerate(src_list):
                        title_1 = title_list[ids]
                        # book_num_1 = book_num_list[ids]
                        title = title_1
                        release_time = date_list[ids]
                        content_src = page_url + src.replace("amp;",'')
                        ...
                        if data_id_rs is not None:
                            ...
                            """
                            # 头部信息 提取书文号，处罚机构、时间从首页提取
                            Label_name = "div"
                            Label_class = "class"
                            Label_class_val = "xxgk_bmxl"
                            ...
                            Label_name_1 = "div"
                            Label_class_1 = "id"
                            Label_class_val_1 = "zoom"

                            cont_soup_2 = cont_soup.find_all('{0}'.format(Label_name_1), attrs={
                                '{0}'.format(Label_class_1): '{0}'.format(Label_class_val_1)})
                            if cont_soup_2 != []:
                                cont_cont = str(cont_soup_2[0]).replace("\n", '').replace("\r", '').replace("\t"
                                                                                                       "", '')
                            else:
                                raise Exception(
                                    """ 对不起全文没有找到<{0} {1}="{2}">这个标签，请查看下具体的情况""".format(Label_name_1, Label_class_1,
                                                                                         Label_class_val_1) + str(
                                        title) + "  " + str(content_src))
                            cont_cont = cont_cont.replace("\r", '').replace('\n', '')

                            # book_number, legal_person, punished_people
                            info = 提取信息.extracting_information_table_5(cont_cont)
                            ...
                            title = title
                            book_num_1 = info[0]
                            if info_header_booknum != '' :
                                book_num = info_header_booknum
                            else:
                                book_num = book_num_1
                            ...
                            penalty_date = release_time # 处罚时间
                            area = ''
                            cont = cont
...
if __name__ == "__main__":
    ...
    index_url = "http://info.hebei.gov.cn/eportal/ui"  #
    table_name = '行政案例数据表'
    # table_name = '行政案例测试表'
    save_path = "E:\行政案例附件\datafolder\河北省\政府信息公开栏"
    source_library = '河北省>政府信息公开栏'
