...
# 访问网址：get
    ...
    def pare_page_data(self, page_url, table_name, save_path, source_library, province):
        ...
        for page_no in range(0, 5):
            ...
            data= {
                '_cimake': 'false',
                'pageNumber': '{0}'.format(page_no),
                'trid': '4417196'

            }

            response = self.get_page_data(page_url_1, data=data)
            # break
            print(response)
            """
            ...
            rs_soup = rs_soup.find_all('form', attrs={'id': 'form1'})
            rs_soup = str(rs_soup[0]).replace("\n", '').replace("\r", '').replace("\t", '').replace(" ", '')
            rs_list = re.findall('<a.*?href="(.*?)".*?>.*?</span>(.*?)</a><span.*?>(.*?)</span>', rs_soup)
            if rs_list:
                for i in rs_list:
                    src_list.append(i[0])
...
if __name__ =="__main__":
    ...
    index_url = "http://safety.xa.gov.cn/ptl/def/def/index_1394_8855.jsp?trid=4417196&_cimake=false&pageNumber=0"  #
    table_name = '行政案例数据表'
    # table_name = '行政案例测试表'
