...
# 访问网址：get
    ...
    def pare_page_data(self, page_url, table_name, save_path, source_library, province):
        ...
        for page_no in range(6, 10):
            src_list, date_list, title_list, law_enforcement_list, book_num_list, punished_institution_list = [], [], [], [], [], []
            if page_no == 0:
                page_url_1 = page_url
            else:
            ...
            # response = self.get_page_data(page_url_1, data=req_data)
            res = 动态访问.get_index_page(page_url_1)
            # break
            ...
            提取页面信息: 
                注意：在信用中国陕西上面，界面没有具体的标题，只有：书文号、处罚名称、相对人称、处罚机构、处罚日期
                当取出的处罚名称为字符”空“的时候，使用相对人名称为标题，不为空的时候使用
            
            """

            response = str(response).replace("\n", '').replace("\r", '').replace("\t", '').replace(" ", '')

            rs_list = re.findall('{"xy020109":"(.*?)".*?"xy020101":"(.*?)".*?"id":"(.*?)".*?"xy020102":"(.*?)","xy020107":"(.*?)".*?"xy010101":"(.*?)".*?}', response)
            print(rs_list)
            print(len(rs_list))
            # if rs_list:
            #     for i in rs_list:
            ...
            #             # 注意该网址的访问地址不规则，使用已经封装好的方法后还需要处理下
            #             content_src = returnSRC().returnSrc(page_url_1, src, '')
            #             content_src = content_src.replace(".jsp?", '_ci_').replace("=", '_')+".html"
            #             # data_id 用于区分这条数据的唯一性（取这条数据请求地址最后一个反斜杠后面的字符串）
            #             data_id = content_src[content_src.rfind("/") + 1:]
            #
            #             """
            #
            #             提取详细数据页面的相关信息: 详细数据页面需要动态访问。且因为全文是要给图片，而且发布时间和处罚时间一致，处罚机构都是：市安监局。其他信息都在图片中就不去特地提取了
            #
            #             """
            #             cont = 动态访问.get_index_page(content_src)
            #             cont_soup = BeautifulSoup(cont, 'lxml')
            #             cont_soup = cont_soup.find_all('div', attrs={'id': 'divContent'})
            #             if cont_soup:
            #                 cont = str(cont_soup[0])
            #             else:
            #                 raise Exception("对不起全文不是一个divContent的div，请查看下具体的情况 ")
            #             # print(cont)
...
if __name__ == "__main__":
    ...
    index_url = "http://www.sxcredit.gov.cn/queryPermitPublishPage.jspx"  #
    # table_name = '行政案例数据表'
    table_name = '行政案例测试表'
    save_path = "E:\行政案例附件\datafolder\陕西省\西安市\%s"
    source_library = '陕西省>西安市_安全生产'
