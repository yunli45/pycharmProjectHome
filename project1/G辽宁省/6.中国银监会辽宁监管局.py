...
class Utils(object):
    ...
    def __init__(self):
        ...
        self.page_regular_expression = '<div.*?>.*?<a.*?href="(.*?)".*?>(.*?)</a></div>.*?<div.*?>(.*?)</div>'
        self.page_label_type = 'div'
        self.page_label_selector = 'class'
        self.page_label_selector_name = 'pcjg_fffbg'

        # 这一组用于提取详细数据页面的信息
        self.cont_label_type = 'div'
        self.cont_label_selector = 'class'
        self.cont_label_selector_name = 'Section1'
            ...
            if src_list:
                for ids, src in enumerate(src_list):
                    ...
                    if title == '中国证券监督管理委员会行政处罚听证规则':
                        ...
                        cont = 预处理模块.dispose_of_data_table(str(cont))
                        need_table_infor = 提取信息.extracting_information_table(title, str(cont))
...
if __name__ =="__main__":
    ...
    index_url = "http://www.cbrc.gov.cn/zhuanti/xzcf/getPcjgXZCFDocListDividePage/dalian.html?current=1"  #
    # table_name = '行政案例数据表'
    table_name = '行政案例测试表'
    save_path = "E:\行政案例附件\datafolder\辽宁省\中国银监会辽宁监管局\%s"
    source_library = '辽宁省>中国银监会辽宁监管局'
