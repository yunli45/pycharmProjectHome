...
# 访问网址：get
    ...
    def pare_page_data(self, page_url, table_name, save_path, source_library, province):
        ...
        # 共计17页
        for page_no in range(1, 136):
            src_list, date_list, title_list = [], [], []
            if page_no == 1:
                page_url_1 = page_url
            else:
                page_url_1 = page_url[:page_url.rfind("-") + 1] + str(page_no) + ".html"
            ...
            rs_soup = rs_soup.find_all('div', attrs={'class': 'right_1'})
            rs_soup = str(rs_soup[0]).replace("\n", '').replace("\r", '').replace("\t", '')
            rs_list = re.findall('<li><a.*?href="(.*?)".*?istitle="true".*?title="(.*?)">.*?</a><span.*?>(.*?)</span>', rs_soup)
            if rs_list:
                for i in rs_list:
                    src_list.append(i[0])
                ...
                if src_list:
                    for ids, src in enumerate(src_list):
                        ...
                        if title.find('实施情况') != -1:
                            ...
                            cont_soup_1 = cont_soup.find_all('div', attrs={'class': 'layout'})
                            if cont_soup_1:
                                cont_1 = str(cont_soup_1[0])
                            else:
                                raise Exception(""" 对不起全文没有找到<div class="layout">这个标签，请查看下具体的情况""" + str(
                            ...
                            info = 提取信息.extracting_information(title, cont_1)
                            print("所有的信息"+str(info))

                            # 浏阳市的 全文内容的标签：<div class="lay_ad_cont">
                            cont_soup_2 = cont_soup.find_all('div', attrs={'class': 'lay_ad_cont'})
                            if cont_soup_2:
                                cont = str(cont_soup_2[0])
                            else:
                                cont = ''
...
if __name__ =="__main__":
    ...
    index_url = "http://www.liuyang.gov.cn/liuyanggov/xxgk/zdxxgk/shgg/xzqlyx/xzcf/e81aa2e0-1.html"  #
    # table_name = '行政案例数据表'
    table_name = '行政案例测试表'
    save_path = "E:\行政案例附件\datafolder\湖南省\浏阳市\%s"
    source_library = '湖南省>浏阳市'
