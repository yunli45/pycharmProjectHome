# coding:utf-8
import re
from J湖南省.工具包 import 链接数据库, 附件下载程序, 判断url前面的点返回完整的请求地址


def dispose_of_data(index_url, content_src, content, save_path, 超链接本地地址):
    content = content
    ...
    if all_p:
        for ids,  p in enumerate(all_p):
            ...
            if p_format_1:
                content = re.sub(p,  '<p align="%s">' % (p_format_1[0][1]),  content)
            elif p_format_2 :
                for pp in p_format_2:
                    p_format_2_1 = re.findall('<p.*?style=".*?(text-align|align):(right|left|center).*?".*?>',  p,  re.I)
                    if p_format_2_1:
                        content = re.sub(p,  '<p align="%s">' % (p_format_2_1[0][1]),  content)
                    else:
    ...
    if all_div:
        for ids,  div in enumerate(all_div):
            ...
            if div_format_1:
                content = re.sub(div,  '<div align="%s">' % (div_format_1[0][1]),  content)
            elif div_format_2:
                for divdiv in div_format_2:
                    div_format_2_1 = re.findall('<div.*?style=".*?(text-align|align):(right|left|center).*?".*?>',  div, flags=re.I)
                    if div_format_2_1:
                        content = re.sub(div,  '<div align="%s">' % (div_format_2_1[0][1]),  content)
                    else:
    ...
    if all_span:
        for ids,  span1 in enumerate(all_span):
            ...
            if span_format_1:
                content = re.sub(span1,  '<span align="%s">' % (span_format_1[0][1]),  content)
            elif span_format_2:
                for span2 in span_format_2:
                    span_format_2_1 = re.findall('<span.*?style=".*?(text-align|align):(right|left|center).*?".*?>',  span1,
                                             flags=re.I)
                    if span_format_2_1:
                        content = re.sub(span1,  '<span align="%s">' % (span_format_2_1[0][1]),  content)
                    else:
                        # print(p)
                        content = re.sub(span1,  '<span>',  content)
            else:
                content = re.sub(span1,  '<span>',  content)
    ...
    if all_table:
        for ids,  table1 in enumerate(all_table):
            ...
            if table_format_1:
                content = re.sub(table1,  '<table align="%s">' % (table_format_1[0][1]),  content)
            elif table_format_2:
                for table2 in table_format_2:
                    table_format_2_1 = re.findall('<table.*?style=".*?(text-align|align):(right|left|center).*?".*?>',
                                              table1,  flags=re.I)
                    if table_format_2_1:
                        content = re.sub(table1,  '<table align="%s">' % (table_format_2_1[0][1]),  content)
                    else:
                        # print(p)
                        content = re.sub(table1,  '<table>',  content)
            else:
                content = re.sub(table1,  '<table>',  content)
    ...
    if all_tr:
        for ids,  tr1 in enumerate(all_tr):
            ...
            if tr_format_1:
                content = re.sub(tr1,  '<tr align="%s">' % (tr_format_1[0][1]),  content)
            elif tr_format_2:
                for tr2 in tr_format_2:
                    tr_format_2_1 = re.findall('<tr.*?style=".*?(text-align|align):(right|left|center).*?".*?>',  tr1,
                                           flags=re.I)
                    if tr_format_2_1:
                        content = re.sub(tr1,  '<tr align="%s">' % (tr_format_2_1[0][1]),  content)
                    else:
                        content = re.sub(tr1,  '<tr>',  content)
            else:
                content = re.sub(tr1,  '<tr>',  content)
    ...
    if all_td:
        for ids,  td1 in enumerate(all_td):
            ...
            if td_format_1:
                content = re.sub(td1,  '<td align="%s">' % (td_format_1[0][1]),  content)
            elif td_format_2:
                for td2 in td_format_2:
                    td_format_2_1 = re.findall('<td.*?style=".*?(text-align|align):(right|left|center).*?".*?>',  td1,
                                           flags= re.I)
                    if td_format_2_1:
                        content = re.sub(td1,  '<td align="%s">' % (td_format_2_1[0][1]),  content)
                    else:
                        # print(p)
                        content = re.sub(td1,  '<td>',  content)
            else:
                content = re.sub(td1,  '<td>',  content)
    ...
    if all_a:
        for a in all_a:
            ...
            if annex_a:
                ...
                # 下载附件
                annex_download_url = 判断url前面的点返回完整的请求地址.returnSRC().returnSrc(index_url, a_hyper, content_src)
    ...
    if all_img:
        for img in all_img:
            ...
            if picture:
                ...
                # 下载图片
                img_download_url = 判断url前面的点返回完整的请求地址.returnSRC().returnSrc(index_url, img_hyper, content_src)
