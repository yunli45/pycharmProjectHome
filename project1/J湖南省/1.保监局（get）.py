...
from 工具包.判断url前面的点返回完整的请求地址 import returnSRC
import time
...
# 访问网址：get
    ...
    def pare_page_data(self, page_url, table_name, save_path, source_library, province):
        ...
        for page_no in range(1, 136):
            ...
            rs_list = re.findall('<a.*?href="(.*?)".*?>(.*?)</a>.*?<td.*?>(.*?)</td>', rs_soup)
            if rs_list:
                for i in rs_list:
                    src_list.append(i[0])
                ...
                if src_list:
                    for ids, src in enumerate(src_list):
                        ...
                        release_time = "20" + date_list[ids].replace("(", '').replace(")", '')   # 发布时间
                        ...
                        if title.find('实施情况') != -1:
                            ...
                            cont_soup = cont_soup.find_all('span', attrs={'id': 'zoom'})
                            if cont_soup:
                                cont = str(cont_soup[0])
                            else:
                                raise Exception(""" 对不起全文没有找到<span class="xilanwb" id="zoom">这个标签，请查看下具体的情况""" + str(
                            ...
                            info = 提取信息.extracting_information(title, cont)
