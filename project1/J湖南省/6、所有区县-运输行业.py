...
# 访问网址：get
    ...
    def pare_page_data(self, page_url, table_name, save_path, source_library, province):
        ...
        for page_no in range(511, 516):
            ...
            rs_soup = rs_soup.find_all('div', attrs={'class': 'mc_content'})
            rs_soup = str(rs_soup[0]).replace("\n", '').replace("\r", '').replace("\t", '')
            # 去除第一个tr
            tr_soup = BeautifulSoup(rs_soup, 'lxml')
            tr_soup = tr_soup.find_all('tr')
            rs_soup = tr_soup[1:]
            rs_list = re.findall('<tr><td.*?>(.*?)</td><td.*?><a.*?href="(.*?)".*?>(.*?)</a></td><td.*?>.*?</td><td.*?>(.*?)</td>.*?</tr>', str(rs_soup))
            if rs_list:
                for i in rs_list:
                    src_list.append(i[1])
                ...
                if src_list:
                    for ids, src in enumerate(src_list):
                        ...
                        # 本网站的标题90%都是”超限运输“四个字，为了防止去重的时候造成影响，现在开始将只是这四个字的标题标题改为：书文号+超限运输
                        if title_1.find("超限运输") == 0:
                            title = book_num_1 + title_1
                        else:
                            title = title_1
                        ...
                        if title.find('实施情况') != -1:
                            ...
                            # 注意需要删除最后一个tr
                            cont_soup_1 = cont_soup.find_all('div', attrs={'class': 'mc_content'})
                            if cont_soup_1 != []:
                                cont = str(cont_soup_1[0])
                            else:
                                raise Exception(""" 对不起全文没有找到<div class="mc_content">这个标签，请查看下具体的情况""" + str(
                            ...
                            info = 提取信息.extracting_information_table_5(cont)
...
if __name__ == "__main__":
    ...
    index_url = "http://218.76.40.80:9000/hnxyfw/xzcf/index.jhtml"  #
    table_name = '行政案例数据表'
    # table_name = '行政案例测试表'
    save_path = "E:\行政案例附件\datafolder\湖南省\运输行业\%s"
    source_library = '湖南省>运输行业'
