...
data = openpyxl.load_workbook(r'E:\Python\PyCharm\project\project1\卫计局\卫计局2(20181025).xlsx')
...
for row in range(2,rows+1) :
    ...
    rowK = 'K%s' % (row)  #
    rowK_va = str(table[rowK].value)
    rowC = 'C%s' % (row)  #
    rowC_va = str(table[rowC].value)
    key = 1
    rowC2  =  'C%s'%(row)
    rowD2  =  'D%s'%(row)
    rowE2  =  'E%s'%(row)
    ...
    RSA1 = re.findall(r'<a.*?>',str(str(rowJ_va)))
    RSA2 = re.findall(r'<img.*?>',str(rowJ_va))
    if RSA1!=[] or RSA2 !=[]:
        if RSA1!=[]:
            RSA3 = re.findall(r'<a.*?href="/datafolder/卫计局数据校正.*?"',str(rowJ_va))
            if RSA3!=[] :
                pass
            else:
                标题1  =  rowC_va
            ...
            RSA4 = re.findall(r'<img.*?href="/datafolder/卫计局数据校正.*?"',str(rowJ_va))
            if RSA4 !=[]:
                pass
            else:
                table2[rowF2] = 标题1
...
data.save(r'E:\Python\PyCharm\project\project1\卫计局\卫计局2(20181025).xlsx')