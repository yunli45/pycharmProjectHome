            ...
            if srcList != []:
                for ids, src in enumerate(srcList):
                    ...
                    if src1.find("http://")!=-1:
                        ...
                        # 匹配文章的头部信息 headInfo
                        # 匹配文章的头部信息 headInfo
                        headInfomation = soup.find_all('div', attrs={'class': 'headInfo'})
                        if headInfomation != []:
                            ...
                            索引号 = 索引号[0]
                            分类 = re.findall('<td.*?>.*?分.*?类.*?</b>(.*?)</td>', str(headInfomation), re.S)
                            分类 = re.sub('<span.*?>', '', str(分类[0])).replace("</span>", '')
                            发布机关 = re.findall('<td.*?>.*?发.*?布.*?机.*?关.*?</b>(.*?)</td>', str(headInfomation), re.S)
                            发布机关 = re.sub('<span.*?>', '', str(发布机关[0])).replace("</span>", '')
                            生成日期 = re.findall('<td.*?>.*?生.*?成.*?日.*?期.*?</b>(.*?)</td>', str(headInfomation), re.S)
                            生成日期 = re.sub('<span.*?>', '', str(生成日期[0])).replace("</span>", '')
                            文号 = re.findall('<td.*?>.*?文.*?号.*?</b>(.*?)</td>', str(headInfomation), re.S)
                            文号 = re.sub('<span.*?>', '', str(文号[0])).replace("</span>", '')
                            主题词 = re.findall('<td.*?>.*?主.*?题.*?词.*?</b>(.*?)</td>', str(headInfomation), re.S)
                            主题词 = re.sub('<span.*?>', '', str(主题词[0])).replace("</span>", '')
                            内容提要 = re.findall('<td.*?>.*?内.*?容.*?提.*?要.*?</b>(.*?)</td>', str(headInfomation), re.S)
                            ...
                            分类 = ''
                            主题词 = ''
                        soupCont1 = soup.find_all('div', attrs={'class': 'Custom_UnionStyle'})
                        ...
                        if soupCont1 != [] or soupCont2 != [] or soupCont3 != [] or soupCont4!=[]:
                            ...
                            sql = " INSERT INTO [环境保护局].[dbo].[%s]([id] ,[来源库名称] ,[来源模块名称] ,[来源模块首页url] ,[标题] ,[发布时间] ,[这条数据的url] ,[这条数据的完整请求url] ,[这条数据属于第几页第几条] ,[插入时间] ,[唯一标识ID] ,[packid] ,[索引号] ,[发布机关] ,[文号] ,[分类] ,[生成日期] ,[主题词] ,[版权所有] ,[全文内容] ,[全文内容2] ,[全文内容3] ,[键值对]  ) VALUES ('%s', '%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s') " % (
                                tableName, id, 来源库名称, 来源模块名称, 来源模块首页url, 标题, 发布时间, 这条数据的url, 这条数据的完整请求url, 这条数据属于第几页第几条,
                                插入时间,
...
if __name__ =="__main__":
    ...
    IndexUrl = "http://www.mee.gov.cn/gzfw_13107/zcfg/zcfgjd/index.shtml"
    SavePath = 'F:\环保局\政策法规解读\%s'
    tableName = '环境保护局'
    超链接本地地址 = '/datafolder/环保局/政策法规解读/'
    来源库名称 = '中华人民共和国生态环境部>政策法规解读'
    版权归属 = '中华人民共和国生态环境部'
    来源模块名称 = '政策法规解读'
    res =   AdminiStrative.parePage(IndexUrl,SavePath,tableName,超链接本地地址,来源库名称,来源模块名称,版权归属)
