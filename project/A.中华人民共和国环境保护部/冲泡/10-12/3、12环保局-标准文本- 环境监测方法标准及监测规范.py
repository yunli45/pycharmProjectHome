...
# 用于解析具体的页面：判断初始的src是否有跳转、是否是附件、以及具体页面数据的提取
    def parePage(self,indexUrl,SavePath,tableName,超链接本地地址,来源库名称,来源模块名称,版权归属):
        ...
        总页数 = 32
        # 注意23页有一个doc格式没处理直接跳过，从24页开始爬取
        for pageNo in range(24,总页数+3):
...
if __name__ =="__main__":
    ...
    IndexUrl = "http://kjs.mee.gov.cn/hjbhbz/bzwb/jcffbz/index.shtml"
    SavePath = 'F:\环保局\标准文本\%s'
    tableName = '环境保护局'
    超链接本地地址 = '/datafolder/环保局/标准文本/'
    来源库名称 = '中华人民共和国生态环境部>环境保护标准>标准文本>环境监测方法标准及监测规范'
    版权归属 = '中华人民共和国生态环境部'
    来源模块名称 = '标准文本'
