                    ...
                    if str(contResponse) == '<Response [404]>':
                        ...
                        if headInfomation1 != []  or headInfomation2!=[]:
                            if headInfomation1!=[]:
                                ...
                                索引号 = re.findall(r'<div><span>.*?索.*?引.*?号.*?</span>(.*?)</div>', str(headInfomation))
                                索引号 = 索引号[0]
                                分类 = re.findall(r'<div><span>.*?分.*?类.*?</span>(.*?)</div>', str(headInfomation))
                                分类 = 分类[0]
                                发布机关 = re.findall(r'<div><span>.*?发.*?布.*?机.*?关.*?</span>(.*?)</div>', str(headInfomation))
                                发布机关 = 发布机关[0]
                                生成日期 = re.findall(r'<div><span>.*?生.*?成.*?日.*?期.*?</span>(.*?)</div>', str(headInfomation))
                                生成日期 = 生成日期[0]
                                文号 = re.findall(r'<div><span>.*?文.*?号.*?</span>(.*?)</div>', str(headInfomation))
                                文号 = 文号[0]
                                主题词 = re.findall(r'<div><span>.*?主.*?题.*?词.*?</span>(.*?)</div>', str(headInfomation))
                                主题词 = 主题词[0]
                        else:
                            索引号 = ''
                            发布机关 = ''
...
if __name__ =="__main__":
    ...
    IndexUrl = "http://kjs.mee.gov.cn/hjbhbz/bzwb/index.shtml"
    SavePath = 'F:\环保局\标准文本\%s'
    tableName = '环境保护局'
    超链接本地地址 = '/datafolder/环保局/标准文本/'
    来源库名称 = '中华人民共和国生态环境部>环境保护标准>标准文本'
    版权归属 = '中华人民共和国生态环境部'
    来源模块名称 = '标准文本'
