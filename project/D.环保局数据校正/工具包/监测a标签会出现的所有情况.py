...
def check_hyper_link(excel_file_path, file_path):
    data_table_rows = open_excel_file(excel_file_path)
    data = data_table_rows[0]
    table = data_table_rows[1]
    rows = data_table_rows[2]
    ...
    # for row in range(2100, 2200):
    # for row in range(2500, rows+1):
    for row in range(2200, 2301):
        # title = table['E{0}'.format(row)].value
        title = table['E%s'%(row)].value
        ...
        dispose__hyper_link1 = dispose__hyper_link(title, now_cont, file_path)
        # cont, 附件不在链接没改, 附件不在链接修改了, 附件存在链接没改
        now_cont = dispose__hyper_link1[0]
        print("处理附件后的全文now_cont"+title+"\n\n"+str(now_cont))
        附件不在链接没改1 = dispose__hyper_link1[1]
        附件不在链接修改了1 = dispose__hyper_link1[2]
        附件存在链接没改1 = dispose__hyper_link1[3]
        附件不在链接没改.append(附件不在链接没改1)
        附件不在链接修改了.append(附件不在链接修改了1)
        附件存在链接没改.append(附件存在链接没改1)

        # 处理过后的数据重新存放在新的四列中，每个单元格寸3万个字符，超出的放入下一个单元格
        if 0 < len(now_cont) < 30000:
            table['AA%s'%(row)] = now_cont

        elif 0 < len(now_cont) < 60000:
            table['AA%s' % (row)] = now_cont[:30000]
            table['AB%s' % (row)] = now_cont[30000:]

        elif 0 < len(now_cont) < 90000:
            table['AA%s' % (row)] = now_cont[:30000]
            table['AB%s' % (row)] = now_cont[30000:60000]
            table['AC%s' % (row)] = now_cont[60000:]

        elif 0 < len(now_cont) < 120000:
            table['AA%s' % (row)] = now_cont[:30000]
            table['AB%s' % (row)] = now_cont[30000:60000]
            table['AC%s' % (row)] = now_cont[60000:90000]
            table['AD%s' % (row)] = now_cont[90000:]

        else:
            print("这条数据好像超出12万个字符。貌似存在问题，请查看下啊" + str(title)+"\n")

    data.save(excel_file_path)
    print("附件不在链接没改"+str(附件不在链接没改))
    print("附件不在链接修改了"+str(附件不在链接修改了))
    print("附件存在链接没改"+str(附件存在链接没改))
...
def processing_label_format(cont):
    ...
    if all_p != []:
        for ids, p in enumerate(all_p):
            ...
            if p_label_format1 != []:
                cont = re.sub(p, '<p align="%s">' % (p_label_format1[0][1]), cont)
                ...
                for pp in p_label_format2:
                    ...
                    if p_label_format2_1:
                        cont = re.sub(p, '<p align="%s">' % (p_label_format2_1[0][1]), cont)
                    else:
                        # print(p)
                        cont = re.sub(p, '<p>', cont)
            else:
                cont = re.sub(p, '<p>', cont)
    ...
    if all_div != []:
        for ids, div in enumerate(all_div):
            ...
            if div_label_format1 != []:
                cont = re.sub(div, '<div align="%s">' % (div_label_format1[0][1]), cont)
                ...
                for divdiv in div_label_format2:
                    ...
                    if div_label_format2_1:
                        cont = re.sub(div, '<div align="%s">' % (div_label_format2_1[0][1]), cont)
                    else:
                        # divrint(div)
                        cont = re.sub(div, '<div>', cont)
            else:
                cont = re.sub(div, '<div>', cont)
    ...
    if all_span != []:
        for ids, span1 in enumerate(all_span):
            ...
            if span_label_format1 != []:
                cont = re.sub(span1, '<span align="%s">' % (span_label_format1[0][1]), cont)
                ...
                for span2 in span_label_format2:
                    ...
                    if span_label_format2_1:
                        cont = re.sub(span1, '<span align="%s">' % (span_label_format2_1[0][1]), cont)
                    else:
                        # print(p)
                        cont = re.sub(span1, '<span>', cont)
            else:
                cont = re.sub(span1, '<span>', cont)
...
excel_file_path = 'D:\Python\PyCharm 20181.4\project\project1\环保局数据校正\环境保护-校对完整版-上送版.xlsx'
