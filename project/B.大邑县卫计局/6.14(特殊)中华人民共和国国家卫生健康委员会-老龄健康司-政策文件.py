...
class Utils(object):
    ...
    def __init__(self):
        ...
        self.compilePageTable = r'<li.*?>.*?<a.*?href="(.*?)".*?title="(.*?)">.*?</a><span .*?>(.*?)</span></li>'
...
# 用于解析具体的页面：判断初始的src是否有跳转、是否是附件、以及具体页面数据的提取
    def parePage(self,indexUrl,SavePath,tableName,超链接本地地址,来源库名称,来源模块名称,版权归属):
        ...
        for pageNo in range(1,总页数+1):
            ...
            print(每页数据集合)
            srcList ,titleList,timeList= [], [], []
            for sr in 每页数据集合:
                srcList.append(sr[0])
                titleList.append(sr[1])
                timeList.append(sr[2])
                ...
                for ids,src in enumerate(srcList):
                    ...
                    if title.find("视频")!=-1 or title.find("焦点访谈")!=-1:
                        ...
                        if dataBaseMaxID == None:
                            dataBaseMaxID = 0
                        id = dataBaseMaxID+1
                        sql = " INSERT INTO [卫计局数据校正].[dbo].[%s]([id], [来源库名称], [来源模块名称], [来源模块首页url], [标题], [类型], [发布时间], [实施时间], [这条数据的url], [这条数据的完整请求url], [这条数据属于第几页第几条], [插入时间],  [packid], [书文号], [标准号], [标准名], [content], [版权归属]) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', NULL , '%s', '%s', '%s', '%s', '%s','%s', '%s', '%s', '%s', '%s') "%( tableName,id,来源库名称,来源模块名称,来源模块首页url,title,类型,发布时间,这条数据的url,这条数据的完整请求url ,这条数据属于第几页第几条,插入时间 ,packid,书文号,标准号,标准名,content,版权归属)
                        print(sql)
...
if __name__ =="__main__":
    ...
    url = "http://www.nhfpc.gov.cn/lljks/zcwj2/new_list.shtml"
    SavePath = 'F:\卫计局\老龄健康司\政策文件\%s'
    tableName = '大邑县卫生局数据'
    超链接本地地址 = '/datafolder/卫计局数据校正/老龄健康司/政策文件/'
    来源库名称 = '老龄健康司>政策文件'
    版权归属 = '中华人民共和国国家卫生健康委员会'
    来源模块名称 = '政策文件'
    res =   AdminiStrative.parePage(url,SavePath,tableName,超链接本地地址,来源库名称,来源模块名称,版权归属)
