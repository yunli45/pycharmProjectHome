        ...
        for pageNo in range(1,总页数+1):
            ...
            if srcList !=[]:
                for ids,src in enumerate(srcList):
                    # 查找数据库种最大id
                    dataBaseMaxID = self.readTableRecord('select max(id) from 大邑县卫生局数据1')[0]
                    ...
                    if title.find("视频")!=-1 or title.find("焦点访谈")!=-1:
                        ...
                        content = 预处理模块.disposeOfData(indexUrl,contenSrc,str(soupCont[0]),SavePath)
...
if __name__ =="__main__":
    ...
    url = "http://www.nhfpc.gov.cn/caiwusi/zcwj2/new_zcwj.shtml"
    SavePath = 'F:\卫计局\财务司\政策文件\%s'
