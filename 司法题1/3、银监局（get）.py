                ...
                if src_list:
                    for ids, src in enumerate(src_list):
                        ...
                        if title.find('实施情况') != -1:
                            ...
                            cont_soup_2 = cont_soup.find_all('table', attrs={'class': 'MsoTableGrid'})
                            if cont_soup_1 != [] and cont_soup_2 == []:
                                cont = str(cont_soup_1[0])
                            elif cont_soup_2 != [] and cont_soup_1 == []:
                                cont = str(cont_soup_2[0])
                            else:
                                raise Exception(""" 对不起全文没有找到<table class="MsoNormalTable"> 和 <table class="MsoTableGrid">这个标签，请查看下具体的情况""" + str(
...
if __name__ =="__main__":
    ...
    index_url = "http://www.cbrc.gov.cn/chinese/home/docViewPage/60150407&current=1"  #
    table_name = '行政案例数据表'
    # table_name = '行政案例测试表'
    save_path = "E:\行政案例附件\datafolder\湖南省\银监局\%s"
    source_library = '湖南省>银监局'
