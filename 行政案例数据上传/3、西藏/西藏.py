...
def check_hyper_link(table_name, excel_file_path, adj_file_path, adj_orr_excel_path):
    ...
    #         title = ws['C%s' % (row)].value  # 标题
    #         if title == None:
    #             break
    #         else:
    #             cont = str(ws['I%s' % (row)].value)   # 全文，后边处理后的，传入数据库的字段为：content
    #             cont = cont.replace("None", '')
    #
    #             source_url =  ws['J%s' % (row)].value                        # 这条数据的完整请求地址
    #
    ...
    #                 ws['X%s' % (row)] = char_num
                ...
                #     ws['I%s' % (row)] = cont_end[:30000]
                #     ws['T%s' % (row)] = cont_end[30000:]
                #     ws['AA%s' % (row)] = char_num
                # elif 60000 <= len(cont_end) < 90000:
                #     ws['S%s' % (row)] = cont_end[:30000]
                #     ws['T%s' % (row)] = cont_end[30000:60000]
                #     ws['U%s' % (row)] = cont_end[60000:]
                #     ws['AA%s' % (row)] = char_num
                # elif 90000 <= len(cont_end) < 120000:
                #     ws['S%s' % (row)] = cont_end[:30000]
                #     ws['T%s' % (row)] = cont_end[30000:60000]
                #     ws['U%s' % (row)] = cont_end[60000:90000]
                #     ws['V%s' % (row)] = cont_end[90000:]
                #     ws['AA%s' % (row)] = char_num
    ...
    #             ws['Y{0}'.format(row)] = the_server_adj_list
    #             ws['Z{0}'.format(row)] = the_local_adjlist
    #     wb.save(excel_file_path)
    ...
    if answer == "yes":
        ...
        for row in range(2, rows+1):
            ...
            if title == None:
                ...
                book_num = str(ws['D%s' % (row)].value).replace("None", '')              # 文号
                legal_person = str(ws['F%s' % (row)].value).replace("None", '')          # 法人
                punished_person = str(ws['E%s' % (row)].value).replace("None", '')       # 被处罚主体
                law_organ = str(ws['G%s' % (row)].value).replace("None", '')             # 执法机构（自收录数据库中没有这个字段）
                penalty_time = str(ws['H%s' % (row)].value).replace("None", '').replace(" 00:00:00", '').replace("-", '')          # 处罚日期
                category = str(ws['S%s' % (row)].value).replace("None", '')              # 类别
                region = str(ws['T%s' % (row)].value).replace("None", '')                # 区域
                law = str(ws['U%s' % (row)].value).replace("None", '')                   # 机构
                Objectofpunishment = str(ws['W%s' % (row)].value).replace("None", '')    # 处罚对象
                cont = str(ws['I%s' % (row)].value)                                      # 全文，后边处理后的，传入数据库的字段为：content
                cont = cont.replace("None", '')
                # char_num = len(re.sub('</？[^>]*>', '', cont))  # 文章字数
                char_num = str(ws['X{0}'.format(row)].value).replace("None", '') # 文章字数
                adj = str(ws['Y{0}'.format(row)].value).replace("None", '').replace(' ','')        # 附件
...
if __name__ == '__main__':
    ...
    check_hyper_link(table_name, "./西藏【校对-上传版】.xlsx", '', './西藏附件情况.xlsx')