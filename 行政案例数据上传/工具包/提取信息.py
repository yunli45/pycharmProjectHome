...
# 银监局（大部分是表格，少部分会出出现全文是内容的情况） http://www.cbrc.gov.cn/chinese/home/docViewPage/60180406&current=1
def extracting_information_table_12(cont):
    cont = str(cont).strip("\n \t").replace("\n", '').replace("\t", '')
    cont_soup = BeautifulSoup(cont, 'lxml')
    cont_soup = cont_soup.find_all('div', attrs={'class': 'Section1'})
    if cont_soup !=[]:
        content = cont_soup[0]
    else:
        raise Exception("sorry, 全文内容不在<div class='Section1'>这个标签中，请看下呢！！！！！！！")
    # 全文是一个表格。
    if content.find("table")!=-1:
        content_soup = BeautifulSoup(content, 'lxml')
        table_a = "table"
        table_b = "class"
        table_c = "MsoNormalTable"
        content_soup = content_soup.find_all('{0}', attrs={'{1}': '{2}'}).format(table_a, table_b, table_c)
        if content_soup !=[]:
            # 由于空tr的存在不好根据td来判断处罚人、法人、处罚单位、时间等，先以 行政处罚决定书文号 这一行作为基准判断其他的数据


            pass
        else:
            raise  Exception("sorry, 全文内容不在<{0} {1}='{2}'>这个标签中，请看下呢！！！！！！！").format(table_a, table_b, table_c)
    else:
        # 全文是一个完整的内容
        pass
    area = ''
