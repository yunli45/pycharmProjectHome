...
def check_hyper_link(table_name, excel_file_path, adj_file_path, adj_orr_excel_path):
    ...
    #             break
    #         else:
    #             cont = str(ws['S%s' % (row)].value) + str(ws['T%s' % (row)].value) + str(ws['U%s' % (row)].value) + str(ws['V%s' % (row)].value)  # 全文，后边处理后的，传入数据库的字段为：content
    ...
    #             pass
    #         else:
    #             cont = str(ws['S%s' % (row)].value) + str(ws['T%s' % (row)].value) + str(ws['U%s' % (row)].value) + str(
    #                 ws['V%s' % (row)].value)  # 全文，后边处理后的，传入数据库的字段为：content
    #             cont = cont.replace("None", '')
    ...
    #             ws['AB{0}'.format(row)] = the_server_adj_list
    #             ws['AC{0}'.format(row)] = the_local_adjlist
    #     wb.save(excel_file_path)
    ...
    if answer == "yes":
        ...
        for row in range(2, rows+1):
            ...
            if title == None:
                ...
                law = str(ws['P%s' % (row)].value).replace("None", '')                   # 机构
                region = str(ws['O%s' % (row)].value).replace("None", '')                # 区域
                category = str(ws['N%s' % (row)].value).replace("None", '')              # 类别
                Objectofpunishment = str(ws['Z%s' % (row)].value).replace("None", '')    # 处罚对象
                cont = str(ws['S%s' % (row)].value) + str(ws['T%s' % (row)].value) + str(ws[
                    'U%s' % (row)].value) + str(ws[
                    'V%s' % (row)].value) # 全文，后边处理后的，传入数据库的字段为：content
                cont = cont.replace("None", '')
                char_num = len(re.sub('</？[^>]*>', '', cont))  # 文章字数
                adj = str(ws['AB{0}'.format(row)].value).replace("None", '').replace(' ','')        # 附件
...
if __name__ == '__main__':
    ...
    check_hyper_link(table_name, "./海南省【校对上传版】.xlsx", 'E:\行政案例附件\datafolder\历史\海南省', './海南省附件情况.xlsx')